...
async function updateMatch(
  ...
  if (updates.status === 'accepted') {
    const userA = data.user_a as any;
    const userB = data.user_b as any;

    // Notify both users asynchronously
    sendPushNotification(
      supabaseClient,
      [userId === userA.id ? userB.id : userA.id],
      '¡Nuevo Match!',
      `Has hecho match con ${userId === userA.id ? userB.display_name : userA.display_name}. ¡Di hola!`,
      { screen: 'Matches' }
    );
  }
    ...
    try {
      ...
      if (method === 'POST') {
        ...
        if (existing) {
          ...
          if (existing.status === 'pending' && existing.user_b_id === userId) {
            ...
            const { error: chatError } = await supabaseClient
              .from('chats')
              .insert({ match_id: existing.id });

            if (chatError && !chatError.message.includes('duplicate key')) {
              console.error('Failed to auto-create chat:', chatError);
            }

            const response: ApiResponse<Match> = { data: updatedMatch as Match };
            return new Response(JSON.stringify(response), {
              status: 200,
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            });
