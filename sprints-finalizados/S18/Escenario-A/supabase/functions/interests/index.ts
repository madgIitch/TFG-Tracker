...
async function enrichInterestsWithProfiles(rows: InterestRow[]): Promise<RoomInterest[]> {
  ...
  const profileById = new Map((profiles as ProfileLite[]).map((item) => [item.id, item]))
  return rows.map((row) => ({
    ...row,
...
async function getUserInterests(userId: string): Promise<RoomInterest[]> {  
  ...
  return enrichInterestsWithProfiles(rows)
}  
...
async function createInterest(interestData: RoomInterestCreateRequest): Promise<RoomInterest> {  
  ...
  const rows = [data as InterestRow]
  const enriched = await enrichInterestsWithProfiles(rows)
  return enriched[0] as RoomInterest
}  
