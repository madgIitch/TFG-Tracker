...
import { INTERESES_OPTIONS, ZONAS_OPTIONS, lifestyleLabelById } from '../constants/swipeFilters';
import type { Profile, ProfilePhoto } from '../types/profile';
import type { Flat, Room, RoomExtras } from '../types/room';
import { getStyles } from '../styles/screens/ProfileDetailScreen.styles';
...
export const ProfileDetailScreen: React.FC<ProfileDetailScreenProps> = ({
  ...
  const handleDeleteAccount = () => {
    Alert.alert(
      'Eliminar Cuenta',
      '¿Estás seguro de que quieres eliminar tu cuenta de forma permanente? Esta acción es irreversible y se perderán todos tus datos.',
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          text: 'Sí, eliminar',
          ...
          onPress: () => {
            Alert.alert(
              'Confirmación Final',
              'Esta es la última advertencia. ¿Realmente deseas eliminar tu cuenta para siempre?',
              [
                { text: 'Cancelar', style: 'cancel' },
                {
                  ...
                  onPress: async () => {
                    try {
                      ...
                      Alert.alert('Error', 'No se pudo eliminar la cuenta. Por favor, inténtalo de nuevo más tarde.');
                      setLoading(false);
                    }
                  },
                },
    ...
    useCallback(() => {
      ...
      if (routeProfile) {
        ...
        if (routeProfile.id && routeProfile.id !== currentUserId) {
          profilePhotoService
            ...
            .then((data) => {
              if (isMounted) setProfilePhotos(data);
            })
            .catch((error) =>
      ...
      const fetchOwnData = async () => {
        try {
          ...
          console.error('Error cargando perfil o fotos:', error);
          if (isMounted) Alert.alert('Error', 'No se pudo cargar el perfil');
        } finally {
  ...
  const lifestyleItems = [
    ...
    ...(profile.lifestyles ?? []).map(id => lifestyleLabelById.get(id) ?? id)
  ];
    ...
    <View style={styles.container}>
        ...
        {isOwnProfile ? (
          <View style={styles.headerActions}>
            <TouchableOpacity
              ...
              onPress={() => {
                themeContext.setThemeMode(themeContext.mode === 'dark' ? 'light' : 'dark');
              }}
            >
              <Ionicons name={themeContext.mode === 'dark' ? 'sunny-outline' : 'moon-outline'} size={18} color="#111827" />
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.headerIconButton}
        ...
        {activeTab === 'perfil' && (
          <>
            {carouselPhotos.length > 0 ? (
              <View style={styles.heroContainer}>
                  ...
                  <View style={styles.heroPagination}>
                    {carouselPhotos.map((_, i) => (
                      <View
                        ...
                        style={[
                          styles.heroDot,
                          i === activePhotoIndex && styles.heroDotActive,
                        ]}
                      />
                    ))}
                  </View>
                ...
                <View style={styles.heroOverlayContainer}>
                  <View style={styles.heroCapsule}>
                    ...
                    />
                    <View style={styles.heroCapsuleContent}>
                      <Text style={styles.heroName}>
                        {profile.display_name ?? 'Usuario'}
                        {birthDateValue ? `, ${birthDateValue.replace(' años', '')}` : ''}
                      </Text>
                      {preferredZoneLabels.length > 0 && (
                        <Text style={styles.heroLocation}>
                          <Ionicons name="location-sharp" size={14} color="#FFF" /> {preferredZoneLabels[0]}
                        </Text>
                      )}
                      <View style={styles.heroBadgesRow}>
                        {memberSinceYear ? (
                          <View style={styles.heroBadgeClear}>
                            <Ionicons name="shield-checkmark" size={12} color="#FFF" />
                            <Text style={styles.heroBadgeTextClear}>Desde {memberSinceYear}</Text>
                          </View>
                        ) : null}
                        {housingBadge ? (
                          <View style={[styles.heroBadgeClear, styles.heroBadgeAccent]}>
                            <Text style={styles.heroBadgeTextClear}>{housingBadge}</Text>
                          </View>
                        ) : null}
                      </View>
                    </View>
              ...
              <View style={styles.heroContainer}>
                <View style={[styles.heroImage, styles.avatarPlaceholder]}>
                  <Ionicons name="person" size={60} color="#9CA3AF" />
                </View>
                <View style={styles.heroOverlayContainer}>
                  <View style={[styles.heroCapsule, { backgroundColor: 'rgba(0,0,0,0.85)' }]}>
            ...
            {isOwnProfile && (
              <View style={styles.section}>
                ...
                <View style={styles.manageCard}>
                  <View style={styles.manageInfo}>
                    <Text style={[styles.manageTitle, { color: theme.colors.text }]}>Modo Oscuro</Text>
                  </View>
                  <Switch
                ...
                <TouchableOpacity
                  style={[styles.manageCard, { marginTop: 12, backgroundColor: theme.colors.surface, borderColor: theme.colors.error, borderWidth: 1 }]}
                  onPress={handleDeleteAccount}
                >
                  <View style={styles.manageInfo}>
                    <Text style={[styles.manageTitle, { color: theme.colors.error }]}>Eliminar cuenta</Text>
                    <Text style={[styles.manageSubtitle, { color: theme.colors.error }]}>
      ...
      {!isOwnProfile && !isFromMatch && (
        <View style={styles.bottomActions}>
          ...
          <TouchableOpacity
            style={[styles.bottomButton, styles.likeButton]}
            onPress={() => navigation.goBack()}
          >
            <View style={[styles.glassTint, styles.likeTint]} />
            <Ionicons name="heart" size={28} color="#7C3AED" />
          </TouchableOpacity>
        </View>
      )}
