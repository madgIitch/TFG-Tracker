...
import { getStyles } from '../styles/screens/ChatScreen.styles';
...
export const ChatScreen: React.FC = () => {  const theme = useTheme();
  ...
  const orderedMessages = useMemo(() => {
    const list: (Message | { id: string; type: 'date'; text: string })[] = [];
    let lastDate = '';

    messages.forEach((msg) => {
      const dateObj = new Date(msg.rawCreatedAt);
      const isToday = new Date().toDateString() === dateObj.toDateString();
      const isYesterday = new Date(Date.now() - 86400000).toDateString() === dateObj.toDateString();

      let dateString = '';
      if (isToday) dateString = 'Hoy';
      else if (isYesterday) dateString = 'Ayer';
      else {
        dateString = dateObj.toLocaleDateString('es-ES', { day: 'numeric', month: 'short' });
      }

      if (dateString !== lastDate) {
        list.push({ id: `date-${msg.id}`, type: 'date', text: dateString });
        lastDate = dateString;
      }
      list.push(msg);
    });
  ...
  const renderMessage = ({ item }: { item: Message | { id: string; type: 'date'; text: string } }) => {
    if ('type' in item) {
      return (
    ...
    return (
      ...
      >
        ...
        >
          <Text style={[styles.bubbleText, isMine ? styles.bubbleTextDark : styles.bubbleTextLight]}>
            {item.text}
          </Text>
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={{ flex: 1 }}>
        ...
        {showNewMessageIndicator && (
          <TouchableOpacity style={styles.newMessagePill} onPress={scrollToBottom}>
            <Text style={styles.newMessagePillText}>Mensajes nuevos</Text>
