...
function matchesFilters(
  ...
  if (filters.zones && filters.zones.length > 0) {
    ...
    // No excluimos estrictamente si fallan las zonas, son ponderadas ahora.
...
export function calculateProfileCompatibilityScore(
  ...
  if (
    (seekerProfile.housing_situation === 'seeking' && targetProfile.housing_situation === 'offering') ||
    (seekerProfile.housing_situation === 'offering' && targetProfile.housing_situation === 'seeking')
  ) {
    score += 25;
    breakdown['housing'] = 25;
  }
  ...
  if (seekerZones.length > 0 && targetZones.length > 0) {
    const commonZones = seekerZones.filter(z => targetZones.includes(z));
    if (commonZones.length > 0) {
      score += 10; // legacy zones points
      breakdown['zones'] = 10;
    }
  }

  // Real Locations Weighted Filters
  if (seekerProfile.preferred_city_id && targetProfile.preferred_city_id) {
    if (seekerProfile.preferred_city_id === targetProfile.preferred_city_id) {
      score += 15;
      breakdown['city'] = 15;
      
      const sZoneIds = seekerProfile.preferred_zone_ids || [];
      const tZoneIds = targetProfile.preferred_zone_ids || [];
      const commonZoneIds = sZoneIds.filter(z => tZoneIds.includes(z));
      if (commonZoneIds.length > 0) {
        score += 10;
        breakdown['exact_zones'] = 10;
      }
    } else {
      // Cercanía por centroides (TODO: Requiere obtener centroides de BBDD)
      // Si son diferentes ciudades, no suma puntos pero tampoco penaliza estrictamente.
    }
  }

  const hasBudgets = seekerProfile.budget_min != null && seekerProfile.budget_max != null &&
                     targetProfile.budget_min != null && targetProfile.budget_max != null;

  if (hasBudgets) {
    const sMin = seekerProfile.budget_min!;
    const sMax = seekerProfile.budget_max!;
    const tMin = targetProfile.budget_min!;
    const tMax = targetProfile.budget_max!;
    
    if (Math.max(sMin, tMin) <= Math.min(sMax, tMax)) {
      const overlap = Math.min(sMax, tMax) - Math.max(sMin, tMin);
      const range = Math.max(sMax, tMax) - Math.min(sMin, tMin) || 1;
      const points = Math.round((overlap / range) * 20);
      score += points;
      breakdown['budget'] = points;
    }
  }

  const sInterests = seekerProfile.interests || [];
  const tInterests = targetProfile.interests || [];
  const commonInterests = sInterests.filter(i => tInterests.includes(i)).length;
  const interestPts = Math.min(25, commonInterests * 5);
  score += interestPts;
  breakdown['interests'] = interestPts;

  let lifestylePts = 0;
  const sLife = seekerProfile.lifestyle_preferences || {};
  const tLife = targetProfile.lifestyle_preferences || {};
  if (sLife.schedule && sLife.schedule === tLife.schedule) lifestylePts += 3;
  if (sLife.cleaning && sLife.cleaning === tLife.cleaning) lifestylePts += 3;
  if (sLife.guests && sLife.guests === tLife.guests) lifestylePts += 4;
  
  const sLifestylesChips = seekerProfile.lifestyles || [];
  const tLifestylesChips = targetProfile.lifestyles || [];
  const commonLifestyles = sLifestylesChips.filter(l => tLifestylesChips.includes(l)).length;
  // Add 2 points per matching lifestyle chip, up to 10 points
  const chipLifePts = Math.min(10, commonLifestyles * 2);

  score += lifestylePts + chipLifePts;
  breakdown['lifestyle'] = lifestylePts + chipLifePts;
  ...
  async (req: Request, payload: JWTPayload): Promise<Response> => {
    ...
    try {
      ...
      for (const profile of filteredProfiles) {
        ...
        const { score: compatibilityScore, breakdown } = calculateProfileCompatibilityScore(
          seekerProfile,
          profile
        );
