  ...
  try {
    ...
    if (flats && flats.length > 0) {
      ...
      if (rooms && rooms.length > 0) {
        ...
        const { data: roomPhotos } = await supabase.from('room_photos').select('photo_url').in('room_id', roomIds)
        
        if (roomPhotos && roomPhotos.length > 0) {
          const filesToRemove = roomPhotos.map(rp => {
            const parts = rp.photo_url.split('/')
            return `${parts[parts.length - 2]}/${parts[parts.length - 1]}` // asumiendo roomId/fileName
          }).filter(Boolean)
