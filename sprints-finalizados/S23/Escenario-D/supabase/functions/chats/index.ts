  ...
  try {
    ...
    if (chatData?.match_id) {
      const { data: matchData } = await supabaseClient
        ...
        .eq('id', chatData.match_id)
        .single();

      if (matchData) {
        const recipientId = matchData.user_a_id === senderId ? matchData.user_b_id : matchData.user_a_id;
        const senderName = data.sender?.display_name || 'Alguien';
