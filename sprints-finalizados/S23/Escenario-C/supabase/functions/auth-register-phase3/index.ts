...
async function findAuthUserByEmail(
  ...
  for (let page = 1; page <= 5; page += 1) {
    ...
    const match = users.find((user: { email?: string }) =>
...
async function rollbackCreatedUser(
  ...
  const client = supabaseClient as LooseSupabaseClient
  await client.from('room_assignments').delete().eq('assignee_id', userId)
  await client.from('profiles').delete().eq('id', userId)
  await client.from('users').delete().eq('id', userId)
  await client.auth.admin.deleteUser(userId)
}

async function ensureChatForMatch(
  supabaseClient: unknown,
  matchId: string
): Promise<void> {
  const client = supabaseClient as LooseSupabaseClient
  ...
  const existingChat = chatData as { id: string } | null
  if (existingChat?.id) return

  await client.from('chats').insert(({ match_id: matchId } as unknown) as never)
}
...
async function createMatchesForFlatmates(
  ...
  for (const flatmateId of flatmateIds) {
    const { data: existingMatchRaw } = await client
      .from('matches')
      .select('id, status')
      .or(
      ...
      .limit(1)
    const existingMatch = existingMatchRaw as Array<{ id: string; status: string }> | null

    if (existingMatch && existingMatch.length > 0) {
      if (existingMatch[0].status === 'accepted') {
        await ensureChatForMatch(client, existingMatch[0].id)
      }
      continue
    }

    const { data: createdMatchRaw } = await client
      .from('matches')
      .insert(({
        user_a_id: userId,
      ...
      .select('id')
      .single()
    const createdMatch = createdMatchRaw as { id: string } | null

    if (createdMatch?.id) {
      await ensureChatForMatch(client, createdMatch.id)
    }
  }
}
  ...
  try {  
    ...
    if (tempData.is_google_user) {
      if (!body.access_token) {
        return new Response(
          JSON.stringify({ error: 'Missing Google session token' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }

      const authUser = await findAuthUserByEmail(supabaseClient, tempData.email)
      if (!authUser) {
        return new Response(
          JSON.stringify({ error: 'Google auth user not found for email' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }
      ...
      if (userError) {
        console.error('User upsert error (google):', userError)
        return new Response(
          JSON.stringify({ error: 'Failed to upsert user record for Google user' }),
      ...
      const { error: profileError } = await supabaseClient
        .from('profiles')
        .upsert({ id: userId, gender: tempData.gender }, { onConflict: 'id' })

      if (profileError) {
        console.error('Profile upsert error (google):', profileError)
        return new Response(
          JSON.stringify({ error: 'Failed to upsert profile for Google user' }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }
      ...
      if (inviteContext) {
        ...
        if (assignmentError) {
          console.error('Assignment creation error (google):', assignmentError)
          return new Response(
      ...
      if (userError) {  
        ...
        await rollbackCreatedUser(supabaseClient, authData.user.id)
        return new Response(  
