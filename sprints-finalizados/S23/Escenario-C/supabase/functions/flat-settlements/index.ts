  ...
  withAuth(async (req: Request, payload: JWTPayload): Promise<Response> => {
    ...
    if (req.method === 'POST') {
      ...
      try {
        ...
        try {
          const tokens = await getTokensForUsers([input.to_user]);
          const address = await getFlatAddress(input.flat_id);
          if (tokens.length > 0) {
            await sendPush({
              tokens,
              title: 'Deuda saldada',
              body: `${input.amount} EUR · ${address ?? 'Piso'}`,
              data: {
                screen: 'FlatSettlements',
          ...
          console.error('[flat-settlements] push error:', pushError);
        }
