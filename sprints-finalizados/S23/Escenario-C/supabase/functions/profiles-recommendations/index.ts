...
type RecommendationContext = {
  cityCoordinatesByName: Map<string, CityCoordinates>;
  selectedNormalizedCities: Set<string>;
  offeringCityByOwnerId: Map<string, string>;
};
...
const CITY_NEARBY_RADIUS_KM = 80;
...
const mapCityCoordinatesFromRow = (
  row: GenericRow
): { normalizedName: string; coordinates: CityCoordinates } | null => {
...
const getProfilePreferredCityName = (profile: Profile): string | null => {
  const socialLinks = profile.social_links;
  if (!socialLinks || typeof socialLinks !== 'object') {
    return null;
  }
  const cityName = socialLinks.preferred_city_name;
...
const getDistanceKm = (a: CityCoordinates, b: CityCoordinates) => {
...
const hasNearbyCityMatch = (
  normalizedCity: string,
  selectedNormalizedCities: Set<string>,
  cityCoordinatesByName: Map<string, CityCoordinates>
) => {
  const cityCoordinates = cityCoordinatesByName.get(normalizedCity);
  if (!cityCoordinates) return false;

  for (const selectedCity of selectedNormalizedCities) {
    const selectedCoordinates = cityCoordinatesByName.get(selectedCity);
    if (!selectedCoordinates) continue;
    if (getDistanceKm(cityCoordinates, selectedCoordinates) <= CITY_NEARBY_RADIUS_KM) {
      return true;
    }
  }
...
const matchesSelectedCities = (
  ...
  if (selectedNormalizedCities.has(normalizedCity)) {
    return true;
  }

  return hasNearbyCityMatch(
    normalizedCity,
    selectedNormalizedCities,
    cityCoordinatesByName
  );
};

const resolveCityCoordinates = async (
  cityName: string
): Promise<{ normalizedName: string; coordinates: CityCoordinates } | null> => {
  const trimmedName = cityName.trim();
  if (!trimmedName) return null;

  const queryCandidates = [trimmedName, `%${trimmedName}%`];
  ...
  for (const candidate of queryCandidates) {
    ...
    const rows = (data as GenericRow[])
      .map(mapCityCoordinatesFromRow)
      .filter(
        (
          item
        ): item is { normalizedName: string; coordinates: CityCoordinates } =>
          Boolean(item)
      );
    ...
    const normalizedInput = normalizeLocationToken(trimmedName);
    const exact = rows.find((row) => row.normalizedName === normalizedInput);
...
const buildCityCoordinatesMap = async (cityNames: string[]) => {
  const normalizedUnique = Array.from(
    new Set(
      cityNames
        .map((name) => name.trim())
        .filter(Boolean)
        .map((name) => normalizeLocationToken(name))
    )
  );

  const map = new Map<string, CityCoordinates>();

  for (const normalizedName of normalizedUnique) {
    const originalName = cityNames.find(
      (name) => normalizeLocationToken(name) === normalizedName
    );
    if (!originalName) continue;

    const city = await resolveCityCoordinates(originalName);
    if (!city) continue;
    map.set(normalizedName, city.coordinates);
  }

  return map;
};

const isSwipeVisible = (profile: Profile): boolean => {
  const socialLinks = profile.social_links;
  if (!socialLinks || typeof socialLinks !== 'object') {
    return true;
  }
  return socialLinks.swipe_visible !== false;
};
...
function matchesFilters(
  ...
  options?: {
    skipBudgetForOffering?: boolean;
    recommendationContext?: RecommendationContext;
    skipCityForOffering?: boolean;
  }
): boolean {
  ...
  if (filters.city && filters.city.length > 0) {
    if (options?.skipCityForOffering && profile.housing_situation === 'offering') {
      // For owners, city is determined by flat location later in room-based filtering.
    } else {
    const preferredCity = getProfilePreferredCityName(profile);
    if (!preferredCity) {
      return false;
    }

    const recommendationContext = options?.recommendationContext;
    const selectedNormalizedCities = recommendationContext?.selectedNormalizedCities;

    if (!selectedNormalizedCities || selectedNormalizedCities.size === 0) {
      const normalizedProfileCity = normalizeLocationToken(preferredCity);
      const exactMatch = filters.city.some(
        (city) => normalizeLocationToken(city) === normalizedProfileCity
      );
      if (!exactMatch) return false;
    } else {
      const matchesCity = matchesSelectedCities(
        preferredCity,
        selectedNormalizedCities,
        recommendationContext?.cityCoordinatesByName ?? new Map<string, CityCoordinates>()
      );
...
function calculateProfileCompatibilityScore(
  ...
  cityCoordinatesByName: Map<string, CityCoordinates>,
  resolveProfileCity: (profile: Profile) => string | null
): {
  score: number;
  breakdown: {
    housing_complementarity: number;
    preferred_zones: number;
    location_affinity: number;
  ...
  const POINTS = {
    ...
    preferredZones: 20,
    locationAffinityExact: 10,
    locationAffinityNearby: 5,
    budgetOverlap: 20,
    sharedInterests: 25,
    lifestyleFit: 10,
  } as const;
  ...
  const seekerCity = resolveProfileCity(seekerProfile);
  const targetCity = resolveProfileCity(targetProfile);
  let locationAffinity = 0;

  if (seekerCity && targetCity) {
    const seekerNormalized = normalizeLocationToken(seekerCity);
    const targetNormalized = normalizeLocationToken(targetCity);

    if (seekerNormalized === targetNormalized) {
      locationAffinity = POINTS.locationAffinityExact;
    } else if (
      hasNearbyCityMatch(
        targetNormalized,
        new Set([seekerNormalized]),
        cityCoordinatesByName
      )
    ) {
      locationAffinity = POINTS.locationAffinityNearby;
    }
  }
...
const handler = withAuth(
  async (req: Request, payload: JWTPayload): Promise<Response> => {
    ...
    try {
      ...
      const selectedFilterCities = (filters?.city ?? []).filter(
      ...
      }).filter((profile) => isSwipeVisible(profile));

      const offeringOwnerIds = Array.from(
        new Set(
          normalizedProfiles
      ...
      if (seekerProfile.housing_situation === 'offering') {
        offeringOwnerIds.push(userId);
      }

      const uniqueOfferingOwnerIds = Array.from(new Set(offeringOwnerIds));
      const offeringCityByOwnerId = new Map<string, string>();
      if (uniqueOfferingOwnerIds.length > 0) {
        const { data: offeringRooms } = await supabaseClient
          .from('rooms')
          .select('owner_id, flat:flats(city)')
          .in('owner_id', uniqueOfferingOwnerIds)
          .eq('is_available', true);

        (offeringRooms ?? []).forEach((room) => {
          const flatData = Array.isArray(room.flat) ? room.flat[0] : room.flat;
          const roomCity =
            typeof flatData?.city === 'string' ? flatData.city.trim() : '';
          if (roomCity && !offeringCityByOwnerId.has(room.owner_id)) {
            offeringCityByOwnerId.set(room.owner_id, roomCity);
          }
        });
      }

      const resolveProfileCity = (profile: Profile): string | null => {
        if (profile.housing_situation === 'offering') {
          const roomCity = offeringCityByOwnerId.get(profile.id);
          if (roomCity) return roomCity;
        }
        return getProfilePreferredCityName(profile);
      };

      const seekerCityName = resolveProfileCity(seekerProfile as Profile);
      const profileCities = normalizedProfiles
        .map(resolveProfileCity)
        .filter((city): city is string => Boolean(city));
      const cityCoordinatesByName = await buildCityCoordinatesMap([
        ...selectedFilterCities,
        ...profileCities,
        ...(seekerCityName ? [seekerCityName] : []),
      ]);
      const selectedNormalizedCities = new Set(
        selectedFilterCities.map((city) => normalizeLocationToken(city))
      );
      const recommendationContext: RecommendationContext = {
        cityCoordinatesByName,
        selectedNormalizedCities,
        offeringCityByOwnerId,
      };

      let filteredProfiles = normalizedProfiles.filter((profile) =>
        matchesFilters(profile, filters, {
          skipBudgetForOffering: true,
          recommendationContext,
          skipCityForOffering: true,
        })
      );
      ...
      if (seekerProfile.housing_situation === 'offering') {
        ...
        const hasCityFilter = selectedFilterCities.length > 0;
        ...
        const shouldRunOfferingRoomFilter =
          hasCityFilter ||
          (filters?.housingSituation === 'offering' &&
            (hasBudgetFilter || hasRuleFilters || hasRoomCountFilter));

        if (shouldRunOfferingRoomFilter) {
          const min =
          ...
          if (ownerIds.length > 0) {
            ...
            if (hasBudgetFilter && filters?.housingSituation === 'offering') {
            ...
            if (roomsError) {
              ...
              (roomsInRange ?? []).forEach((room) => {
                ...
                if (hasCityFilter) {
                  ...
                  const matchesCity = matchesSelectedCities(
                    roomCity,
                    recommendationContext.selectedNormalizedCities,
                    recommendationContext.cityCoordinatesByName
                  );
                  if (!matchesCity) {
                    return;
                  }
              ...
              const filteredByRooms = filteredProfiles.filter((profile) => {
                ...
                return ownersWithRooms.has(profile.id);
              });
