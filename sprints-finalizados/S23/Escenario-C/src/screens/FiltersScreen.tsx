...
export const FiltersScreen: React.FC = () => {
  ...
  useEffect(() => {
    setDraft(filters);
    setSelectedCities(
      (filters.city ?? []).map((cityName) => ({
        id: cityName,
        name: cityName,
      }))
    );
    setSelectedZones(
      (filters.zones ?? []).map((zoneName) => ({
        id: zoneName,
        name: zoneName,
      }))
    );
  ...
  const handleApply = async () => {
    ...
    let ageMin = clamp(draft.ageRange[0], AGE_MIN, AGE_MAX);
    let ageMax = clamp(draft.ageRange[1], AGE_MIN, AGE_MAX);
    ...
    await setFilters({
      ...
      city: selectedCities.map((city) => city.name),
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.header, { paddingTop: headerTopPadding }]}>
        ...
        <View style={styles.titleWrap}>
          ...
          <Text style={styles.activeCountText}>
            {countActiveFilters(draft)} activos
          </Text>
        </View>
      ...
      <ScrollView
        ...
        scrollEnabled={!isDraggingBudget && !isDraggingAge}
      >
          ...
          <>
            ...
            <FormSection title="Genero" iconName="people-outline">
              <PremiumLockWrapper
                ...
                onUpgradePress={openUpgradeModal}
              >
        ...
        <FormSection title="Rango de edad" iconName="hourglass-outline">
          <PremiumLockWrapper
          ...
          >
            <View style={styles.budgetValues}>
              <Text style={styles.budgetValue}>Min: {draft.ageRange[0]} años</Text>
              <Text style={styles.budgetValue}>Max: {draft.ageRange[1]} años</Text>
            </View>
            <BudgetRange
              styles={styles}
              min={AGE_MIN}
        ...
        <FormSection title="Ciudad" iconName="business-outline">
          <LocationSearchSelect
            ...
            onSelect={(option) => {
              setSelectedCities((prev) => {
                if (prev.some((city) => city.name === option.label || city.id === option.id)) {
                  return prev;
                }
          ...
          />
          <View style={styles.locationSelectionWrap}>
            {selectedCities.map((city) => (
              <TouchableOpacity
                ...
                onPress={() =>
                  setSelectedCities((prev) =>
                    prev.filter((item) => item.id !== city.id && item.name !== city.name)
                  )
                }
              >
            ...
            {selectedCities.length === 0 ? (
              <Text style={styles.locationHint}>Sin ciudades seleccionadas.</Text>
            ) : null}
          </View>
        </FormSection>
        ...
        <FormSection title="Habitaciones" iconName="bed-outline">
          <ChipGroup
            ...
            selectedIds={draft.roomCount.map(String)}
            onSelect={(id) => {
              const value = Number(id);
              setDraft((prev) => ({
                ...prev,
                roomCount: prev.roomCount.includes(value)
                  ? prev.roomCount.filter((item) => item !== value)
                  : [...prev.roomCount, value],
              }));
            }}
            multiline
          />
            ...
            onSelect={(id) => {
              setDraft((prev) => ({
                ...
                userType: prev.userType.includes(id)
                  ? prev.userType.filter((item) => item !== id)
                  : [...prev.userType, id],
              }));
            }}
            multiline
        ...
        <FormSection title="Zonas" iconName="map-outline">
          <LocationSearchSelect
            ...
            helperText={
              selectedCities.length === 0
                ? 'Selecciona al menos una ciudad para listar sus zonas.'
                : selectedCities.length > 1
                ? `Usando zonas de ${selectedCities[0].name}.`
                : undefined
            }
            onSearch={async (query) => {
              if (selectedCities.length === 0) return [];
              const city = selectedCities[0];
              const zones = await locationService.getZonesByCity(city.id);
              const normalized = query.trim().toLowerCase();
              const filtered = normalized
                ? zones.filter((zone) => zone.name.toLowerCase().includes(normalized))
                : zones;
          ...
          <View style={styles.locationSelectionWrap}>
            {selectedZones.map((zone) => (
              ...
              >
                <Text style={styles.locationChipText}>{zone.name}</Text>
                <Ionicons name="close" size={12} color={theme.colors.textSecondary} />
              </TouchableOpacity>
            ))}
            {selectedZones.length === 0 ? (
              <Text style={styles.locationHint}>Sin zonas seleccionadas.</Text>
            ) : null}
          </View>
          {selectedCities.length === 0 ? (
            <ChipGroup
  ...
  return (
    ...
    >
      ...
      <View style={styles.sliderLabels}>
        ...
        <Text style={styles.sliderLabel}>{labels[2]}</Text>
      </View>
    </View>
  );
};
