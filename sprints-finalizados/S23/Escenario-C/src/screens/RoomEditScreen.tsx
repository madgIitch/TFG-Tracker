...
import { createStyles } from '../styles/screens/RoomEditScreen.styles';
...
export const RoomEditScreen: React.FC = () => {
  ...
  const loadFlatRooms = useCallback(async (flatId: string) => {
    try {
      ...
      const extras = await roomExtrasService.getExtrasForRooms(
        filtered.map((flatRoom) => flatRoom.id)
      );
      setFlatRoomExtras(
        Object.fromEntries(
          extras.map((extra) => [
  ...
  const defaultTitle = useMemo(() => {
    ...
    const count = flatRooms.reduce((acc, existingRoom) => {
      const extras = flatRoomExtras[existingRoom.id];
      if (roomCategory === 'habitacion') {
      ...
      if (existingRoom.title?.toLowerCase().startsWith(baseLabel.toLowerCase())) {
        return acc + 1;
      }
