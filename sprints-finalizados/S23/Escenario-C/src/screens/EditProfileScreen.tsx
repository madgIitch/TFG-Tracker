...
const ESTILO_VIDA_OPTIONS = [
  ...
  { id: 'tiene_mascota', label: 'Tiene mascota' },
  { id: 'teletrabaja', label: 'Teletrabaja' },
  { id: 'ordenado', label: 'Ordenado' },
];

const LEGACY_LIFESTYLE_ID_BY_LABEL = new Map([
  ['Flexible', 'madrugador'],
  ['Muy limpio', 'ordenado'],
  ['Algunos invitados', 'social'],
]);
...
export const EditProfileScreen: React.FC = () => {
  ...
  const [selectedZones, setSelectedZones] = useState<LocationZone[]>([]);
  const [locationMetadata, setLocationMetadata] = useState<ProfileSocialLinks>({});
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      const nextLifestyleTags = (data.lifestyle_tags ?? [])
        .filter((value): value is string => Boolean(value))
        .slice(0, MAX_LIFESTYLE_TAGS);
      if (nextLifestyleTags.length > 0) {
        setEstiloVida(nextLifestyleTags);
      } else {
        setEstiloVida(
          data.lifestyle_preferences
            ? Object.values(data.lifestyle_preferences)
                .filter((value): value is string => Boolean(value))
                .map((value) => LEGACY_LIFESTYLE_ID_BY_LABEL.get(value) || value)
                .slice(0, MAX_LIFESTYLE_TAGS)
            : []
        );
      }
      ...
      const socialLinks =
        data.social_links && typeof data.social_links === 'object'
          ? data.social_links
          : {};
      setLocationMetadata(socialLinks as ProfileSocialLinks);

      const cityIdFromSocial =
        typeof socialLinks.preferred_city_id === 'string'
          ? socialLinks.preferred_city_id
          : null;
      const cityNameFromSocial =
        typeof socialLinks.preferred_city_name === 'string'
          ? socialLinks.preferred_city_name
          : null;
      const cityId = data.preferred_city_id ?? cityIdFromSocial;
      if (cityId || cityNameFromSocial) {
        setSelectedCity({
          id: cityId ?? cityNameFromSocial ?? 'city',
          name: cityNameFromSocial ?? cityId ?? 'Ciudad',
        });
      } else {
        setSelectedCity(null);
      }

      const zoneNamesFromSocial = Array.isArray(socialLinks.preferred_zone_names)
        ? socialLinks.preferred_zone_names
            .filter((item): item is string => typeof item === 'string' && item.trim().length > 0)
        : [];
      const zoneIdsFromSocial = Array.isArray(socialLinks.preferred_zone_ids)
        ? socialLinks.preferred_zone_ids
            .filter((item): item is string => typeof item === 'string' && item.trim().length > 0)
        : [];

      const zoneNames =
        zoneNamesFromSocial.length > 0
          ? zoneNamesFromSocial
          : (data.preferred_zones ?? []);

      const initialZones = zoneNames.map((name, index) => ({
        id: zoneIdsFromSocial[index] ?? name,
        name,
        city_id: cityId ?? undefined,
        city_name: cityNameFromSocial ?? undefined,
      }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const profileData: Partial<ProfileCreateRequest> = {
        ...
        preferred_zones: selectedZones.map((zone) => zone.name),
        budget_min: presupuestoMinValue,
        budget_max: presupuestoMaxValue,
        social_links: {
          ...locationMetadata,
          preferred_city_id: selectedCity?.id,
          preferred_city_name: selectedCity?.name,
          preferred_zone_ids: selectedZones.map((zone) => zone.id),
          preferred_zone_names: selectedZones.map((zone) => zone.name),
        },
      };
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.sectionBlock}>
          ...
          <Text style={styles.inlineLabel}>Estilo de vida (max. 5)</Text>
          <View style={styles.checkGrid}>
                  ...
                  onPress={() =>
                    setEstiloVida((prev) => {
                      if (prev.includes(option.id)) {
                        return prev.filter((interes) => interes !== option.id);
                      }
        ...
        <View style={styles.sectionBlock}>
          ...
          />
          <LocationSearchSelect
            label="Agregar barrio / zona"
            placeholder={
              selectedCity ? 'Buscar y añadir zona' : 'Primero selecciona ciudad'
            }
            selectedOption={null}
            disabled={!selectedCity}
            helperText={!selectedCity ? 'Selecciona ciudad para buscar zonas.' : undefined}
            onSearch={async (query) => {
              if (!selectedCity?.id) return [];
              const zones = await locationService.getZonesByCity(selectedCity.id);
            ...
            onSelect={(option) => {
              setSelectedZones((prev) => {
                if (prev.some((zone) => zone.id === option.id || zone.name === option.label)) {
                  return prev;
                }
                return [
                  ...prev,
          ...
          />
          <View style={styles.selectedZonesWrap}>
            {selectedZones.map((zone) => (
              <TouchableOpacity
                key={`${zone.id}-${zone.name}`}
                style={styles.selectedZoneChip}
                onPress={() =>
                  setSelectedZones((prev) =>
                    prev.filter((item) => item.id !== zone.id && item.name !== zone.name)
                  )
                }
              >
            ...
            {selectedZones.length === 0 ? (
              <Text style={styles.selectedZoneHint}>No has añadido zonas todavía.</Text>
            ) : null}
          </View>
          </View>
        </View>
