...
import { createRoomEditStyles } from '../styles/screens/RoomEditScreen.styles';
...
export const RoomEditScreen: React.FC = () => {
  ...
  const styles = useMemo(() => createRoomEditStyles(theme), [theme]);
  ...
  const loadFlatRooms = useCallback(async (flatId: string) => {
    try {
      ...
      const extras = await roomExtrasService.getExtrasForRooms(
        filtered.map((roomItem) => roomItem.id)
      );
      setFlatRoomExtras(
        Object.fromEntries(
          extras.map((extra) => [
  ...
  const defaultTitle = useMemo(() => {
    ...
    const count = flatRooms.reduce((acc, roomItem) => {
      const extras = flatRoomExtras[roomItem.id];
      if (roomCategory === 'habitacion') {
      ...
      if (roomItem.title?.toLowerCase().startsWith(baseLabel.toLowerCase())) {
        return acc + 1;
      }
