...
async function sendPushToUsers(
  ...
  }).catch((err: Error) => console.error('[matches] send-push failed:', err));
}
...
async function findExistingMatchBetween(
  userAId: string,
  userBId: string
): Promise<{ id: string; status: string; user_a_id: string; user_b_id: string } | null> {
  ...
  async (req: Request, payload: JWTPayload): Promise<Response> => {
    ...
    try {
      ...
      if (method === 'POST') {
        ...
        const existingMatch = await findExistingMatchBetween(
          matchData.user_a_id,
          matchData.user_b_id
        );
        ...
        if (existingMatch) {
          const isMutualLike =
            ...
            existingMatch.user_a_id === matchData.user_b_id &&
            existingMatch.user_b_id === userId;

          if (isMutualLike) {
            // Target already liked current user → mutual like, accept the match
            const { data: acceptedMatch, error: updateErr } = await supabaseClient
              .from('matches')
              .update({ status: 'accepted' })
              .eq('id', existingMatch.id)
              .select(
                `*, user_a:profiles!matches_user_a_id_fkey(*), user_b:profiles!matches_user_b_id_fkey(*)`
              )
              .single();

            if (updateErr || !acceptedMatch) {
              throw new Error(`Failed to accept match: ${updateErr?.message}`);
            }

            // Create chat automatically
            await supabaseClient.from('chats').insert({ match_id: existingMatch.id });

            // Push notifications to both users
            sendPushToUsers(
              [userId, matchData.user_b_id],
              '¡Nuevo match!',
              'Tenéis un match, id a chatear.',
              { screen: 'Matches' }
            );

            const response: ApiResponse<Match> = { data: acceptedMatch as Match };
            return new Response(JSON.stringify(response), {
              status: 200,
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            });
          ...
          // Match already exists (outgoing pending or already accepted) → idempotent
          const response: ApiResponse<Match> = { data: existingMatch as unknown as Match };
          return new Response(JSON.stringify(response), {
            status: 200,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
      ...
      if (method === 'PATCH') {
        ...
        // Push a ambos usuarios cuando el match es aceptado
        if (updates.status === 'accepted') {
          sendPushToUsers(
            [updatedMatch.user_a_id, updatedMatch.user_b_id],
            '¡Nuevo match!',
            'Tenéis un match, id a chatear.',
            { screen: 'Matches' }
          );
