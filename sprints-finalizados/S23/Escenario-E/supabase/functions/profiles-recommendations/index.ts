...
type CityWithCentroid = {
  id: string;
  centroid?: { lat?: number; lon?: number; latitude?: number; longitude?: number } | null;
};
...
function getCentroidCoords(
  centroid: CityWithCentroid['centroid']
): { lat: number; lon: number } | null {
  if (!centroid) return null;
  const lat = centroid.lat ?? centroid.latitude;
  const lon = centroid.lon ?? centroid.longitude;
  if (lat == null || lon == null) return null;
  return { lat, lon };
}

function nearbyWeightMultiplier(distKm: number): number {
  if (distKm < 50) return 0.75;
  if (distKm < 150) return 0.5;
  return 0;
}
...
function matchesFilters(
  ...
  // User type filter (student = has university, professional = has occupation but no university)
...
function matchesRoomForProfile(profile: Profile, room: RoomWithFlat): boolean {
  ...
  // Usar preferred_zone_ids (nuevo) con fallback a preferred_zones (legacy string[])
  const zoneIds: string[] = (() => {
    const pzi = (profile as Record<string, unknown>).preferred_zone_ids;
    if (Array.isArray(pzi) && pzi.length > 0) return pzi as string[];
    const pz = profile.preferred_zones;
    if (Array.isArray(pz) && pz.length > 0 && typeof pz[0] === 'string') return pz as string[];
    return [];
  })();

  if (zoneIds.length > 0) {
    const districtId = room.flat?.district_id ?? room.flat?.district ?? null;
    if (districtId && !zoneIds.includes(districtId)) {
      return false;
    }
  }
...
function matchesCityFilter(
  roomCityId: string | null | undefined,
  cityFilter: string[]
): boolean {
  if (!cityFilter || cityFilter.length === 0) return true;
  if (!roomCityId) return true;
  return cityFilter.some((c) => c === roomCityId);
}
...
function calculateProfileCompatibilityScore(
  ...
  // Situación habitacional complementaria (+25): uno seeking + otro offering
  ...
  // Zonas preferidas en común (+20): ≥1 zona solapada
  // Usar preferred_zone_ids (nuevo) con fallback a preferred_zones (legacy string[])
  const extractZoneIds = (profile: Profile): string[] => {
    const pzi = (profile as Record<string, unknown>).preferred_zone_ids;
    if (Array.isArray(pzi) && pzi.length > 0) return pzi as string[];
    const pz = profile.preferred_zones;
    if (Array.isArray(pz) && pz.length > 0 && typeof pz[0] === 'string') return pz as string[];
  ...
  const seekerZones = extractZoneIds(seekerProfile);
  const targetZones = extractZoneIds(targetProfile);
  ...
  // Intereses en común (+25): 5 pts por interés compartido, máx 5 intereses
  ...
  // Lifestyle encaja (+10): horario + limpieza + invitados
...
const handler = withAuth(
  async (req: Request, payload: JWTPayload): Promise<Response> => {
    ...
    try {
      ...
      if (seekerProfile.housing_situation === 'offering') {
        ...
        const { data: rooms, error: roomsError } = await supabaseClient
          ...
          .select(
            ...
            price_per_month,
            is_available,
            flat:flats(gender_policy, district, city, city_id, district_id, rules)
          `
          )
          .eq('owner_id', userId)
        ...
        if (
          ...
          if (ownerIds.length > 0) {
            let roomsQuery = supabaseClient
              .from('rooms')
              .select('id, owner_id, price_per_month, flat:flats(rules, city, city_id, district_id, district)')
            ...
            if (roomsError) {
              ...
              (roomsInRange ?? []).forEach((room) => {
                if (!matchesCityFilter(room.flat?.city_id ?? room.flat?.city, cityFilter)) return;
                if (!hasRuleFilters) {
      ...
      // Nearby-city weighting: obtener centroides si el seeker tiene ciudad preferida
      const seekerCityId = (seekerProfile as Record<string, unknown>).preferred_city_id as string | null | undefined;
      let seekerCentroid: { lat: number; lon: number } | null = null;
      const cityCache = new Map<string, { lat: number; lon: number } | null>();

      if (seekerCityId) {
        const { data: seekerCity } = await supabaseClient
          .from('cities')
          .select('id, centroid')
          .eq('id', seekerCityId)
          .single();
        if (seekerCity) {
          seekerCentroid = getCentroidCoords(seekerCity.centroid);
          cityCache.set(seekerCityId, seekerCentroid);
        }
      }

      const preferredCityIds = new Set<string>();
      if (seekerCityId) preferredCityIds.add(seekerCityId);
      // Extraer ciudades preferidas desde preferred_zones jsonb (multi-city)
      const pz = (seekerProfile as Record<string, unknown>).preferred_zones;
      if (Array.isArray(pz) && pz.length > 0 && typeof pz[0] === 'object') {
        (pz as { city_id?: string }[]).forEach((entry) => {
          if (entry.city_id) preferredCityIds.add(entry.city_id);
        });
      }
      ...
      for (const profile of filteredProfiles) {
        ...
        const { score: rawScore, breakdown: compatibilityBreakdown } =
          calculateProfileCompatibilityScore(seekerProfile, profile);
        ...
        // Aplicar nearby-weighting basado en ciudad del perfil/sus rooms
        let compatibilityScore = rawScore;
        if (seekerCentroid && preferredCityIds.size > 0) {
          const profileCityId = (profile as Record<string, unknown>).preferred_city_id as string | null | undefined;
          if (profileCityId && !preferredCityIds.has(profileCityId)) {
            let profileCentroid = cityCache.get(profileCityId) ?? undefined;
            if (profileCentroid === undefined) {
              const { data: city } = await supabaseClient
                .from('cities')
                .select('id, centroid')
                .eq('id', profileCityId)
                .single();
              profileCentroid = city ? getCentroidCoords(city.centroid) ?? null : null;
              cityCache.set(profileCityId, profileCentroid ?? null);
            }
            if (profileCentroid) {
              const dist = haversineKm(seekerCentroid, profileCentroid);
              const multiplier = nearbyWeightMultiplier(dist);
              if (multiplier === 0) {
                // Ciudad demasiado lejos: omitir
                continue;
              }
              compatibilityScore = Math.round(rawScore * multiplier);
            }
          }
        }
