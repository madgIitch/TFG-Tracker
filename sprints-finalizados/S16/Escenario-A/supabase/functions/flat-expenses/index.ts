...
async function getFlatAddress(flatId: string): Promise<string> {
  const { data } = await supabaseAdmin.from('flats').select('address').eq('id', flatId).single();
  return (data?.address as string | null)?.trim() || 'Tu piso';
}
...
serve(
  withAuth(async (req: Request, payload: JWTPayload): Promise<Response> => {
    ...
    if (req.method === 'POST') {
      ...
      const pushRecipients = memberIds.filter((memberId) => memberId !== paidBy);
      void (async () => {
        const tokens = await getTokensForUsers(pushRecipients);
        if (tokens.length === 0) return;
        const flatAddress = await getFlatAddress(nextFlatId);
        fireAndForgetPush({
          tokens,
          title: 'Nuevo gasto compartido',
          body: `${description} - ${Number(amount.toFixed(2)).toFixed(2)} EUR`,
          data: {
            screen: 'FlatExpenses',
            flatId: nextFlatId,
            flatAddress,
        ...
        console.error('[flat-expenses] push trigger error:', error);
      });
