...
async function getFlatAddress(flatId: string): Promise<string | null> {
  const { data, error } = await supabaseAdmin
    .from('flats')
    .select('address')
    .eq('id', flatId)
  ...
  withAuth(async (req: Request, payload: JWTPayload): Promise<Response> => {
    ...
    if (req.method === 'POST') {
      ...
      try {
        ...
        try {
          const pushMemberIds = await getFlatMemberIds(input.flat_id);
          const recipients = (pushMemberIds ?? []).filter((id) => id !== input.paid_by);
          const tokens = await getTokensForUsers(recipients);
          const address = await getFlatAddress(input.flat_id);

          if (tokens.length > 0) {
            await sendPush({
              tokens,
              title: 'Nuevo gasto',
              body: `${input.description} · ${address ?? 'Piso'}`,
              data: {
                screen: 'FlatExpenses',
          ...
          console.error('[flat-expenses] push error:', pushError);
        }
