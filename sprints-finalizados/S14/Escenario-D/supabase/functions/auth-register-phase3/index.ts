...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (body.invitation_code) {
      const { data: invCode, error: invError } = await supabaseClient
        .from('flat_invitation_codes')
        .select('*')
        .eq('code', body.invitation_code)
        .single();

      if (invError || !invCode || !invCode.is_active || invCode.used_count >= invCode.max_uses || new Date(invCode.expires_at) < new Date()) {
        return new Response(
          JSON.stringify({ error: 'Código de invitación inválido o expirado' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      validInvitation = invCode;

      const { data: roomInfo } = await supabaseClient
        .from('rooms')
        .select('flat_id, owner_id')
        .eq('id', invCode.room_id)
        .single();

      if (!roomInfo) {
        return new Response(
          JSON.stringify({ error: 'La habitación asociada no existe' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
    ...
    if (tempData.is_google_user) {
      ...
      // Procesar código de invitación
      if (validInvitation && roomDetails) {
        try {
          // 1. Marcar código como usado
          const newUsedCount = validInvitation.used_count + 1
          await supabaseClient
            .from('flat_invitation_codes')
            .update({
              used_count: newUsedCount,
              last_used_at: new Date().toISOString(),
              is_active: newUsedCount < validInvitation.max_uses
            })
            .eq('id', validInvitation.id)

          // 2. Crear match con el propietario de la habitación
          const newUserId = signInData.user.id;
          const ownerId = roomDetails.owner_id;
          let matchId: string | null = null;

          if (ownerId && newUserId !== ownerId) {
            const userA = ownerId < newUserId ? ownerId : newUserId;
            const userB = ownerId < newUserId ? newUserId : ownerId;

            const { data: ownerMatch } = await supabaseClient
              .from('matches')
              .insert({
                user_a_id: userA,
                user_b_id: userB,
            ...
            if (ownerMatch) matchId = ownerMatch.id;
          }

          // 3. Crear asignación de habitación
          await supabaseClient
            .from('room_assignments')
            .insert({
              room_id: validInvitation.room_id,
              assignee_id: newUserId,
              status: 'accepted',
              match_id: matchId
            })

          // 4. Crear matches con otros inquilinos del mismo piso
          const { data: roomAsgs } = await supabaseClient
            .from('room_assignments')
            .select('assignee_id, rooms!inner(flat_id)')
            .eq('rooms.flat_id', roomDetails.flat_id)

          const flatMates = new Set<string>();
          if (roomAsgs) {
            roomAsgs.forEach((r: any) => {
              if (r.assignee_id && r.assignee_id !== newUserId && r.assignee_id !== ownerId) {
                flatMates.add(r.assignee_id);
              }
            });
          }

          if (flatMates.size > 0) {
            const matchInserts = Array.from(flatMates).map(mateId => ({
              user_a_id: mateId < newUserId ? mateId : newUserId,
              user_b_id: mateId < newUserId ? newUserId : mateId,
              status: 'room_assigned'
            }));
            await supabaseClient.from('matches').insert(matchInserts);
          }
        } catch (postRegError) {
          console.error("Error processing invitation code after auth creation:", postRegError);
          // Don't fail the complete request if the assignment logic has an issue, just log it.
        }
      }
