...
Deno.test("calculateProfileCompatibilityScore - Lifestyles (elementos compartidos -> max 10 pts)", () => {
  const seeker: Profile = {
    ...createBaseProfile(),
    lifestyles: ["deportista", "madrugador", "ordenado"]
  };
  const target: Profile = {
    ...createBaseProfile(),
    lifestyles: ["deportista", "ordenado", "mascotas"]
  };
  
  const result = calculateProfileCompatibilityScore(seeker, target);
  // Shared elements: deportista, ordenado (2 shared)
  // Each gives 2 points normally according to edge function. Since 2 => 4 pts out of 10 max
  assertEquals(result.breakdown["lifestyle"], 4);
});
