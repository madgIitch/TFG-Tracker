...
import { getStyles } from '../styles/screens/RoomManagementScreen.styles';
...
export const RoomManagementScreen: React.FC = () => {  const theme = useTheme();
  ...
  useEffect(() => {
    ...
    const channel = supabaseClient
      .channel('room_management')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'rooms' }, () => {
        if (isMounted) loadRooms();
      })
  ...
  const handleInvite = async (room: Room) => {
    try {
      setLoading(true);
      let codeObj = await roomService.getInvitationCode(room.id);
      if (!codeObj) {
        codeObj = await roomService.generateInvitationCode(room.id);
      }
      const code = codeObj.code;
      Alert.alert(
        'Código de Invitación',
        `Comparte este código con la persona que deseas invitar a la habitación:\n\n${code}`,
        [
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
                    ...
                    <View key={room.id} style={styles.roomCard}>
                      ...
                      {isOwner && (
                        <View style={styles.actionsRow}>
                          ...
                          {!isCommonArea && !isAssigned && (
                            <TouchableOpacity
                              style={styles.actionButton}
                              onPress={() => handleInvite(room)}
                            >
      ...
      {flats.length > 0 && isOwner && !loading && (
        <View style={[styles.actionDock, { bottom: Math.max(insets.bottom, 16) }]}>
          <TouchableOpacity style={styles.addFloatingButton} onPress={handleCreateRoom}>
            <Ionicons name="add" size={20} color={theme.colors.background} />
            <Text style={styles.addFloatingButtonText}>Agregar habitacion</Text>
          </TouchableOpacity>
        </View>
      )}
    </View>
