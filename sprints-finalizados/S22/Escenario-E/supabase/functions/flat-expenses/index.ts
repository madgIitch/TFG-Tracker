...
async function sendPushToUsers(
  ...
  }).catch((err: Error) => console.error('[flat-expenses] send-push failed:', err))
}
...
const handler = withAuth(async (req: Request, payload: JWTPayload): Promise<Response> => {
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      // Push a todos los miembros del piso excepto el pagador (fire-and-forget)
      try {
        const members = await getFlatMembers(body.flat_id)
        const recipients = members.map((m) => m.id).filter((id) => id !== userId)
        if (recipients.length > 0) {
          const descLabel = body.description?.substring(0, 40) ?? 'Nuevo gasto'
          sendPushToUsers(recipients, 'Nuevo gasto registrado', descLabel, {
            screen: 'FlatExpenses',
            flatId: body.flat_id,
            flatAddress: '',
          })
        ...
        console.error('[flat-expenses] push error:', pushErr)
      }
