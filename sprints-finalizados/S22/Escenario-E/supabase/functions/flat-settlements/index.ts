...
async function sendPushToUsers(
  ...
  }).catch((err: Error) => console.error('[flat-settlements] send-push failed:', err))
}
...
const handler = withAuth(async (req: Request, payload: JWTPayload): Promise<Response> => {
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      // Push al acreedor (to_user) notificando que recibió un pago (fire-and-forget)
      if (body.to_user) {
        sendPushToUsers([body.to_user], 'Pago recibido', `Han liquidado ${body.amount}€ contigo.`, {
