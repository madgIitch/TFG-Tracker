...
async function handler(req: Request): Promise<Response> {  
  ...
  try {  
    ...
    if (!body.email || !body.password || !body.data?.first_name || !body.data?.last_name || !body.data?.birth_date || !body.data?.gender) {  
      return new Response(  
        JSON.stringify({       
          error: 'Missing required fields',  
          details: 'email, password, first_name, last_name, birth_date, and gender are required'  
        }),  
        {       
          status: 400,       
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }  
