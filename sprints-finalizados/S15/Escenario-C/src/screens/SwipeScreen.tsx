...
export const SwipeScreen: React.FC = () => {
  ...
  const horizontalGutter = Math.max(12, Math.round(screenWidth * 0.05));
  const cardWidth = screenWidth - horizontalGutter * 2;
  const swipeThreshold = screenWidth * 0.3;
  const cardHeight = clamp(
    Math.round(screenHeight * 0.68),
    360,
    Math.round(screenWidth * 1.6)
  );
  ...
  const handleSwipe = (direction: 'left' | 'right') => {
    ...
    Animated.timing(position, {
      toValue: {
        x:
          direction === 'right'
            ? screenWidth + Math.round(cardWidth * 0.25)
            : -screenWidth - Math.round(cardWidth * 0.25),
        y: 0,
      },
      duration: 240,
    ...
    const loadProfiles = async () => {
      ...
      try {
        ...
        const mapped = recommendations
          .map((rec) => {
            const mappedProfile = mapProfileToSwipe(rec.profile);
            if (!mappedProfile) {
              return null;
            }
            return {
              ...mappedProfile,
              compatibilityPercentage: toCompatibilityPercentage(
  ...
  const renderCard = (profile: SwipeProfile, index: number) => {
    ...
    const stackOffset = isNext ? Math.round(cardHeight * 0.018) : 0;
