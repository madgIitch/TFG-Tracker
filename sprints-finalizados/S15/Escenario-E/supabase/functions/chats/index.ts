...
async function sendPushToUsers(
  ...
  }).catch((err: Error) => console.error('[chats] send-push failed:', err));
}
        ...
        if (type === 'message') {
          ...
          try {
            const { data: chatRow } = await supabaseClient
              ...
              .single();
            if (chatRow?.match) {
              const match = chatRow.match as { user_a_id: string; user_b_id: string };
              const recipientId = match.user_a_id === userId ? match.user_b_id : match.user_a_id;
              const senderName = (message as any).sender?.display_name ?? 'Nuevo mensaje';
              const msgBody = body.body.length > 100 ? body.body.substring(0, 97) + '...' : body.body;
              await sendPushToUsers([recipientId], senderName, msgBody, {
                screen: 'Chat',
                chatId: body.chat_id,
                name: senderName,
                avatarUrl: (message as any).sender?.avatar_url ?? '',
              });
            }
          } catch (pushErr) {
            console.error('[chats] push error:', pushErr);
          }
