...
async function sendPushToUsers(
  ...
  }).catch((err: Error) => console.error('[room-assignments] send-push failed:', err));
}
  ...
  withAuth(async (req: Request, payload: JWTPayload): Promise<Response> => {
    ...
    if (req.method === 'POST') {
      ...
      if (matchId) {
        ...
        // Push al assignee notificando oferta de habitación (fire-and-forget)
        if (assigneeId) {
          try {
            // Intentar obtener el chatId del match para navegar directamente al chat
            const { data: chatRow } = await supabaseAdmin
              .from('chats')
              .select('id')
              .eq('match_id', matchId)
              .maybeSingle();
            const notifData: Record<string, string> = chatRow?.id
              ? { screen: 'Chat', chatId: chatRow.id, name: '', avatarUrl: '' }
              : { screen: 'Matches' };
            sendPushToUsers([assigneeId], 'Oferta de habitación', 'Te han asignado una habitación. Revísala en el chat.', notifData);
          } catch (pushErr) {
            console.error('[room-assignments] push error:', pushErr);
          }
        }
