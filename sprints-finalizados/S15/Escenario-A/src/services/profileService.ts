...
class ProfileService {
  ...
  private normalizeCompatibilityScore(rawScore: number): number {
    ...
    return this.clamp(rawScore, 0, 1);
  }
  ...
  private async getAuthHeaders(): Promise<Record<string, string>> {
    ...
    authService.logTokenDiagnostics('[ProfileService.getAuthHeaders]', token);
  ...
  async getProfile(): Promise<Profile | null> {
    ...
    if (response.status === 401) {
      try {
        ...
        console.log('[ProfileService.getProfile] Initial 401 body:', initialErrorBody);
      } catch {
        const initialErrorText = await response.clone().text();
        console.log('[ProfileService.getProfile] Initial 401 text:', initialErrorText);
      }
    }
      ...
      if (newToken) {
        ...
        if (response.status === 401) {
          try {
            ...
            console.log('[ProfileService.getProfile] Retry 401 body:', retryErrorBody);
          } catch {
            const retryErrorText = await response.clone().text();
            console.log('[ProfileService.getProfile] Retry 401 text:', retryErrorText);
          }
        }
      }
    }
