...
describe('profileService', () => {
  ...
  it('getProfileRecommendations forwards extended filter payload', async () => {
    (globalThis.fetch as jest.Mock).mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => ({
    ...
    expect(body).toContain('"ageRange":[21,33]');
    expect(body).toContain('"city":["sevilla"]');
    expect(body).toContain('"roomCount":[1,2]');
    expect(body).toContain('"userType":["student"]');
  });
});
