...
import { createStyles } from '../styles/screens/RoomDetailScreen.styles';
...
export const RoomDetailScreen: React.FC = () => {
  ...
  const [acceptedAssignments, setAcceptedAssignments] = useState<RoomAssignment[]>([]);
  ...
  useEffect(() => {
    ...
    const subscribeRealtime = async () => {
      ...
      const roomIdFilter = `id=eq.${room.id}`;
      const flatIdFilter = `id=eq.${room.flat_id}`;
      const channel = supabaseClient
        .channel(`rt:room-detail:room:${room.id}:screen:RoomDetail`)
        .on(
          'postgres_changes',
          {
            event: '*',
          ...
          () => {
            ...
            refreshRoom().catch((error) => {
              console.error('Error refrescando detalle por asignaciones realtime:', error);
            });
          }
        )
        .on(
          ...
          () => {
            if (!isMounted) return;
            refreshRoom().catch((error) => {
              console.error('Error refrescando detalle por room realtime:', error);
            });
          }
        )
        .on(
          ...
          () => {
            if (!isMounted) return;
            refreshRoom().catch((error) => {
              console.error('Error refrescando detalle por extras realtime:', error);
            });
          }
        )
        .on(
          ...
          () => {
            if (!isMounted) return;
            refreshRoom().catch((error) => {
              console.error('Error refrescando detalle por flat realtime:', error);
            });
          }
        )
        .subscribe();
    ...
    subscribeRealtime().catch((error) => {
      console.error('Error suscribiendo realtime de RoomDetail:', error);
    });
    ...
    return () => {
      ...
      if (channel) {
        channel.unsubscribe().catch((error) => {
          console.error('Error cancelando realtime de RoomDetail:', error);
        });
        supabaseClient.removeChannel(channel);
      }
    };
  }, [room.id, room.flat_id, refreshRoom]);
  ...
  const primaryCtaLabel = isOwner
    ...
    ? 'Pedir unirse'
    : 'Contactar';
  const secondaryCtaLabel = isOwner
    ? 'Ver matches'
    : statusLabel === 'Disponible'
    ? 'Contactar'
    : 'Ver matches';
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}> 
              ...
              <View style={styles.carouselOverlayRow}>
                {typeLabel ? (
                  <View style={styles.overlayChip}>
                    <Text style={styles.overlayChipText}>{typeLabel}</Text>
                  </View>
                ) : null}
                {!isCommonArea && roomState.price_per_month != null ? (
                  <View style={styles.overlayChip}>
                    <Text style={styles.overlayChipText}>{roomState.price_per_month} EUR/mes</Text>
                  </View>
                ) : null}
                {statusLabel ? (
                  <View style={styles.overlayChip}>
                    <Text style={styles.overlayChipText}>{statusLabel}</Text>
                  </View>
                ) : null}
              </View>
            </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.detailCard}>
            {acceptedAssignments.length === 0 ? (
              <Text style={styles.detailNoteText}>Aún no hay compañeros asignados.</Text>
            ) : (
                ...
                {acceptedAssignments.map((assignment) => (
                  <View key={assignment.id} style={styles.roommateRow}>
                    {assignment.assignee?.avatar_url ? (
                      <Image
                        source={{ uri: assignment.assignee.avatar_url }}
                        style={styles.roommateAvatar}
                      />
                    ) : (
                      <View style={styles.roommateAvatarPlaceholder}>
                        <Ionicons name="person" size={16} color={theme.colors.textTertiary} />
                      </View>
                    )}
                    <View style={styles.roommateInfo}>
                      <Text style={styles.roommateName}>
                        {assignment.assignee?.display_name ?? 'Compañero'}
                      </Text>
                      <Text style={styles.roommateMeta}>Asignado</Text>
                    </View>
                  </View>
                ))}
              </View>
      ...
      <View style={styles.bottomCtaBar}>
        ...
        >
          <Text style={styles.bottomCtaPrimaryText}>{primaryCtaLabel}</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={styles.bottomCtaSecondary}
          onPress={() => navigation.navigate('Main', { screen: 'Matches' })}
        >
          <Text style={styles.bottomCtaSecondaryText}>{secondaryCtaLabel}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
