...
serve(
  withAuth(async (req: Request, payload: JWTPayload): Promise<Response> => {
    ...
    if (req.method === 'POST') {
      ...
      void (async () => {
        const tokens = await getTokensForUsers([toUser]);
        if (tokens.length === 0) return;
        const flatAddress = await getFlatAddress(nextFlatId);
        fireAndForgetPush({
          tokens,
          title: 'Liquidacion actualizada',
          body: 'Se ha registrado una deuda saldada en tu piso.',
          data: {
            screen: 'FlatSettlement',
            flatId: nextFlatId,
            flatAddress,
          },
        });
      })().catch((pushError) => {
        console.error('[flat-settlements] push trigger error:', pushError);
      });
