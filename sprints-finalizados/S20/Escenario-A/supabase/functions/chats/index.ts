...
const CHAT_ENABLED_MATCH_STATUSES = new Set([
  'accepted',
  'room_offer',
  'room_assigned',
  'room_declined',
...
async function createChat(matchId: string, userId: string): Promise<Chat> {
  ...
  if (userId !== match.user_a_id && userId !== match.user_b_id) {
    throw new Error('Unauthorized: You can only create chats for your own matches');
  }

  if (!CHAT_ENABLED_MATCH_STATUSES.has(match.status)) {
    throw new Error('Chat is not available for this match status');
  }
...
async function markMessagesAsRead(chatId: string, userId: string): Promise<void> {
  await getChatMessages(chatId, userId);
  await supabaseClient
    .from('messages')
...
async function triggerChatMessagePush(
  chat: Chat & { match: MatchWithProfiles },
  senderId: string,
  message: Message
): Promise<void> {
  const match = chat.match;
  const recipients = [match.user_a_id, match.user_b_id].filter(
  ...
  const sender = (message as Message & { sender?: { display_name?: string; avatar_url?: string } }).sender;
  fireAndForgetPush({
    tokens,
    title: sender?.display_name?.trim() || 'Nuevo mensaje',
    body: message.body?.trim() || 'Tienes un nuevo mensaje',
    data: {
      screen: 'Chat',
      chatId: chat.id,
      name: sender?.display_name?.trim() || 'Chat',
      avatarUrl: sender?.avatar_url?.trim() || '',
    },
  });
}
    ...
    try {
      ...
      if (method === 'POST') {
        ...
        if (type === 'chat') {
          ...
          const chat = await createChat(body.match_id, userId);
          const response: ApiResponse<Chat> = { data: chat };
          return new Response(JSON.stringify(response), {
            status: 201,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        ...
        if (type === 'message') {
          ...
          if (chat) {
            void triggerChatMessagePush(chat as Chat & { match: MatchWithProfiles }, userId, message).catch((error) => {
              console.error('[chats] push trigger error:', error);
            });
          }
      ...
      if (method === 'DELETE') {
        ...
        if (!chatId || chatId === 'chats') {
          return new Response(JSON.stringify({ error: 'chat_id path parameter is required' }), {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
