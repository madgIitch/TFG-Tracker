...
import { createRoomDetailStyles } from '../styles/screens/RoomDetailScreen.styles';
...
export const RoomDetailScreen: React.FC = () => {
  ...
  const styles = useMemo(() => createRoomDetailStyles(theme), [theme]);
        ...
        if (isMounted) {
          ...
          setRoommates(
            assignmentsResponse.assignments
              ...
              .map((assignment) => assignment.assignee as Profile)
          );
          const assigned =
  ...
  useEffect(() => {
    ...
    const scheduleRefresh = () => {
      ...
      refreshTimer = setTimeout(async () => {
        ...
        try {
          ...
          console.error('Error recargando detalle de habitacion por realtime:', error);
        }
      }, 250);
    };
    ...
    const subscribe = async () => {
      channel = await createRealtimeSubscription(
        buildRealtimeChannelName('room-detail', 'room', room.id, currentUserId),
        [
          {
            filter: {
              event: '*',
    ...
    subscribe().catch((error) => {
      console.error('Error suscribiendo realtime en RoomDetail:', error);
    });
  ...
  const companionProfiles = roommates.filter(
    (roommate, index, array) =>
      array.findIndex((candidate) => candidate.id === roommate.id) === index
  );
  const ctaLabel =
    roomState.owner_id === currentUserId
      ? 'Gestionar anuncio'
      : statusLabel === 'Disponible'
      ? 'Solicitar unirse'
      : 'Contactar por disponibilidad';
  const bottomCtaOffset = insets.bottom + 16;
  const scrollBottomPadding = bottomCtaOffset + 72;
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      >
        <View style={styles.carouselContainer}>
          {photos.length > 0 ? (
            ...
            <View style={styles.carouselEmptyState}>
              ...
              <Text style={styles.carouselEmptyText}>Sin fotos por ahora</Text>
            </View>
          )}
          <View style={styles.heroInfo}>
            <Text style={styles.heroTitle}>{roomState.title}</Text>
            <View style={styles.heroMetaRow}>
              ...
              {roomState.size_m2 != null ? (
                <View style={styles.heroMetaChip}>
                  ...
                  <Text style={styles.heroMetaText}>{roomState.size_m2} m2</Text>
                </View>
              ) : null}
              {statusLabel ? (
                <View style={styles.heroMetaChip}>
            ...
            <View style={styles.detailCard}>
              ...
              <View style={styles.detailRow}>
                <View style={styles.detailLabelRow}>
                  ...
                  <Text style={styles.detailLabel}>Politica convivencia</Text>
                </View>
                <Text style={styles.detailValue}>
                  {flat.gender_policy === 'men_only'
                    ? 'Solo hombres'
                    : flat.gender_policy === 'flinta'
                    ? 'FLINTA'
                    : 'Mixto'}
                </Text>
              </View>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Companeros actuales</Text>
          <View style={styles.detailCard}>
            {companionProfiles.length > 0 ? (
              <View style={styles.roommateList}>
                {companionProfiles.map((roommate) => (
                  <View key={roommate.id} style={styles.roommateItem}>
                    {roommate.avatar_url ? (
                      <Image source={{ uri: roommate.avatar_url }} style={styles.roommateAvatar} />
                    ) : (
                      <View style={styles.roommateFallback}>
                        <Ionicons name="person-outline" size={16} color={theme.colors.textSecondary} />
                      </View>
                    )}
                    <Text style={styles.roommateName}>
                      {roommate.display_name ?? 'Inquilino'}
                    </Text>
                  </View>
                ))}
              </View>
              ...
              <Text style={styles.detailNoteText}>
                Todavia no hay companeros confirmados para esta habitacion.
              </Text>
            )}
          </View>
        </View>
      ...
      <View style={[styles.bottomCtaContainer, { bottom: bottomCtaOffset }]}>
        <TouchableOpacity style={styles.bottomCtaButton} onPress={() => {}}>
