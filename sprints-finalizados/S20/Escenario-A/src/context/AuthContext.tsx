...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {  
  ...
  const checkAuth = async () => {  
    try {  
      ...
      if (token && userJson) {  
        const parsedUser = JSON.parse(userJson) as Partial<User>;
        const userData: User = {
          id: parsedUser.id ?? '',
          ...
          identity_document: parsedUser.identity_document,
          is_premium: Boolean(parsedUser.is_premium),
          created_at: parsedUser.created_at ?? new Date(0).toISOString(),
        };
