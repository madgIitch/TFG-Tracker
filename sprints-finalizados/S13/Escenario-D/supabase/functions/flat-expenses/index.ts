...
async function createFlatExpense(requestData: FlatExpenseCreateRequest): Promise<any> {
    ...
    if (splits && splits.length > 0) {
        const notifyUsers = splits
            .map(s => s.user_id)
            .filter(id => id !== paid_by);

        if (notifyUsers.length > 0) {
            // Get flat details for the address
            const { data: flatData } = await supabaseClient
                .from('flats')
                .select('address')
                .eq('id', flat_id)
                .single();
            ...
            sendPushNotification(
                ...
                notifyUsers,
                'Nuevo gasto añadido',
                `Se ha añadido un nuevo gasto: "${description}" por ${amount}€`,
                {
                    screen: 'FlatExpenses',
                    flatId: flat_id,
                    flatAddress: flatData?.address || ''
                }
            );
        }
    }
