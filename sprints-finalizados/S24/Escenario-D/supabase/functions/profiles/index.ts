...
async function getProfile(userId: string): Promise<Profile | null> {
  const { data, error } = await supabaseClient
    .from('profiles')
    .select('*, users!profiles_id_fkey(birth_date, is_premium)')
    .eq('id', userId)
    .single();
  ...
  const { users, ...profileData } = data as Profile & {
    users?: { birth_date?: string | null; is_premium?: boolean };
  };
  const profile: Profile = {
    ...profileData,
    birth_date: users?.birth_date ?? null,
    is_premium: users?.is_premium ?? false,
  };
  ...
  async (req: Request, payload: JWTPayload): Promise<Response> => {
    ...
    try {
      ...
      if (method === 'POST') {
        ...
        const profile = await createProfile(profileInsertData);
        const response: ApiResponse<Profile> = { data: profile };
      ...
      if (method === 'PATCH') {
        ...
        if (birth_date !== undefined) userUpdates.birth_date = birth_date;
        ...
        if (Object.keys(userUpdates).length > 0) {
          const { error: userUpdateError } = await supabaseClient
            ...
            .eq('id', userId);
          if (userUpdateError) {
            console.error('[profiles] Failed to update users table:', userUpdateError);
          }
        }

        const updatedProfile = await updateProfile(userId, profileUpdates);
        if (
