...
import { getStyles } from '../styles/screens/SwipeScreen.styles';
...
const SkeletonCard = () => {
  ...
  return (
    <Animated.View style={[styles.cardWrap, { opacity, zIndex: 1, width: '100%', height: '100%' }]}>
      <GlassCard style={styles.profileCard}>
        <View style={[styles.profileImage, { backgroundColor: theme.colors.surfaceLight }]} />
      </GlassCard>
    </Animated.View>
  );
};
...
export const SwipeScreen: React.FC = () => {
  ...
  const profiles = useMemo(() => {
    return applyFilters(allProfiles, filters, excludedProfileIds);
  }, [allProfiles, filters, excludedProfileIds]);

  const currentProfile = profiles[0];
  const canSwipe = isPremium || swipesUsed < SWIPE_LIMIT;
    ...
    return {
      ...
      lifestyle: [
        ...(profile.lifestyle_preferences
          ...
          : []),
        ...(profile.lifestyles ? profile.lifestyles.map(id => lifestyleLabelById.get(id) ?? id) : [])
      ],
      profile,
    };
  };
  ...
  useEffect(() => {
    const applyProfileFilters = async () => {
      ...
      try {
        ...
        setIsUserVisible(profile.is_visible !== false);
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <LinearGradient
          colors={isDarkMode ? ['rgba(0,0,0,0.6)', 'rgba(30,30,30,0.8)'] : ['rgba(255,255,255,0.6)', 'rgba(245,245,247,0.8)']}
          style={StyleSheet.absoluteFillObject}
        />
      </ImageBackground>
      ...
      <View style={[styles.content, { paddingTop: insets.top + 10 }]}>
        {!isUserVisible && (
          <View style={{ backgroundColor: theme.colors.warning, padding: 12, borderRadius: 8, marginHorizontal: 20, marginBottom: 16 }}>
            <Text style={{ color: '#111827', fontWeight: 'bold', textAlign: 'center' }}>Tu perfil está pausado</Text>
            <Text style={{ color: '#111827', textAlign: 'center', fontSize: 13, marginTop: 4 }}>
              No aparecerás en las recomendaciones de los demás. Actívalo en "Editar perfil" para hacer match.
            </Text>
          </View>
        )}
      ...
      <GlassCard style={[styles.actionDock, { bottom: insets.bottom + 24 }]}>
        <View style={styles.actionRow}>
          ...
          <ActionButton
            icon="heart"
            onPress={() => (isPremium || canSwipe) ? handleSwipe('right') : setUpgradeModalVisible(true)}
