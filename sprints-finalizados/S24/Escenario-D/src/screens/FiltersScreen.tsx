...
import { getStyles } from '../styles/screens/FiltersScreen.styles';
...
export const FiltersScreen: React.FC = () => {
  ...
  const styles = getStyles(theme);  const navigation = useNavigation<StackNavigationProp<any>>();
  ...
  const activeFiltersCount = useMemo(() => {
    ...
    if ((draft.cityIds?.length || 0) > 0) count++;
    if ((draft.userType?.length || 0) > 0) count++;
    if ((draft.zones?.length || 0) > 0) count++;
    if ((draft.zoneIds?.length || 0) > 0) count++;
      ...
      >
        {profileHousing === 'offering' ? (
          ...
          <>
            ...
            {/* --- Ubicación (Free) --- */}
            <FormSection title="Ubicación" iconName="location-outline" zIndex={50}>
              <CitySearchSelect
                label="Ciudad"
                value={draft.cityIds?.[0] || null}
                onSelect={(id) => {
                  setDraft((prev) => ({
                    ...prev,
                    cityIds: id ? [id] : [],
              ...
              <ZoneSelect
                label="Zonas/Barrios"
                cityId={draft.cityIds?.[0] || null}
                value={draft.zoneIds || []}
                onSelect={(val) => {
                  setDraft((prev) => ({
                    ...prev,
                    zoneIds: val as string[],
                  }));
                }}
                multiple={true}
              />
            ...
            {/* --- Premium Filters --- */}
            <View style={{ opacity: isPremium ? 1 : 0.6 }} pointerEvents={isPremium ? 'auto' : 'box-none'}>
