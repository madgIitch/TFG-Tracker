...
import { INTERESES_OPTIONS, ZONAS_OPTIONS, ESTILO_VIDA_OPTIONS } from '../constants/swipeFilters';
import type {
  ProfileCreateRequest,
  HousingSituation,
  ProfilePhoto,
} from '../types/profile';
import { getStyles } from '../styles/screens/EditProfileScreen.styles';

// No longer used, mapped via constants instead
...
export const EditProfileScreen: React.FC = () => {  const theme = useTheme();
  ...
  const [zonas, setZonas] = useState<string[]>([]); // legacy
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      setZonas(data.preferred_zones || []); // legacy
      setIsVisible(data.swipe_visibility_active !== false);
      const nextMin =
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await setFilters({
        ...
        cityIds: preferredCityId ? [preferredCityId] : [],
        zoneIds: preferredZoneIds,
        zones: zonas, // legacy
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top + 16 }]}>
        ...
        <Text style={[theme.typography.h2, { color: theme.colors.text, flex: 1, marginLeft: 12 }]}>
          Editar perfil
        </Text>
      </View>

      <FormKeyboardWrapper style={styles.content} contentContainerStyle={{ paddingBottom: 100 }}>
          ...
          <View style={styles.sectionCard}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <View style={{ flex: 1, paddingRight: 16 }}>
                <Text style={[styles.inlineLabel, { marginBottom: 4 }]}>Pausar mi perfil</Text>
                <Text style={{ fontSize: 13, color: theme.colors.textSecondary }}>Si lo activas, no aparecerás en Swipe para otros usuarios.</Text>
              </View>
              <Switch
                value={!isVisible}
                onValueChange={(val) => setIsVisible(!val)}
              ...
              {ESTILO_VIDA_OPTIONS.map((option) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[styles.checkItem, isActive && styles.checkItemActive]}
                    onPress={() => {
                        const isSelected = estiloVida.includes(option.id);
                        if (!isSelected && estiloVida.length >= 5) {
                          Alert.alert('Limite', 'Puedes seleccionar hasta 5 estilos de vida.');
                          return;
                        }
                        setEstiloVida((prev) =>
                          isSelected
        ...
        <View style={styles.sectionBlock}>
          ...
          <View style={styles.sectionCard}>
            ...
            <CitySearchSelect
              ...
              initialCityName={preferredCityName || undefined}
              onSelect={(id, name) => {
                setPreferredCityId(id);
                setPreferredCityName(name);
                if (!id) setPreferredZoneIds([]);
              }}
            />
            <ZoneSelect
              label="Zonas/Barrios"
              cityId={preferredCityId}
              value={preferredZoneIds}
              onSelect={(val) => setPreferredZoneIds(val as string[])}
              multiple={true}
            />
          </View>
        </View>
      ...
      <View style={[styles.actionDock, { bottom: Math.max(insets.bottom, 16) }]}>
        <View style={styles.saveButtonFull}>
          <Button
