...
import { getStyles } from '../styles/screens/CreateFlatScreen.styles';

export const CreateFlatScreen: React.FC = () => {  const theme = useTheme();
  ...
  const handleSave = async () => {
    ...
    if (!districtId) {
      Alert.alert('Error', 'Selecciona un distrito / barrio');
      return;
    }
    ...
    try {
      ...
      await roomService.createFlat({
        ...
        city: cityName || 'Desconocida', // legacy
        district: 'ZONA', // legacy fallback if needed
        city_id: cityId,
    ...
    <View style={styles.container}>
      ...
      <FormKeyboardWrapper style={styles.content} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <CitySearchSelect
          label="Ciudad"
          value={cityId}
          initialCityName={cityName || undefined}
          onSelect={(id, name) => {
            setCityId(id);
            setCityName(name);
            if (!id) setDistrictId(null);
          }}
        />
        <ZoneSelect
          ...
          onSelect={(val) => {
             ...
             setDistrictId(selected.length > 0 ? selected[0] : null);
          }}
          multiple={false}
        />
