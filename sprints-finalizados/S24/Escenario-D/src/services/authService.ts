  ...
  async register(userData: RegisterRequest): Promise<AuthResponse> {
    ...
    if (domain) {
      try {
        ...
        if (dnsJson.Status === 3 || (dnsJson.Status === 0 && (!dnsJson.Answer || dnsJson.Answer.length === 0))) {
           throw new Error('El dominio del correo electronico no existe o no puede recibir correos. Verifica que este bien escrito.');
        }
      } catch (e) {
        if (e instanceof Error && e.message.includes('dominio')) throw e;
        console.warn('DNS validation bypassed due to network error:', e);
      }
    }
