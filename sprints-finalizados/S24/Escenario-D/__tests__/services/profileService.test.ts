...
describe('profileService', () => {
  ...
  it('deleteAccount calls fetch with DELETE method and correct URL', async () => {
    ...
    const deleteCall = (global.fetch as jest.Mock).mock.calls.find(call => call[1].method === 'DELETE');
    expect(deleteCall).toBeDefined();
    expect(deleteCall[0]).toMatch(/\/account-delete$/);
  });

  it('deleteAccount throws error on failure', async () => {
    (global.fetch as jest.Mock).mockResolvedValueOnce({
      ok: false,
      status: 400,
    ...
    const { profileService } = require('../../src/services/profileService');
    await expect(profileService.deleteAccount()).rejects.toThrow('Delete failed');
  });
});
