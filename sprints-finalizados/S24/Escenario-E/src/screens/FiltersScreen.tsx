...
import { makeStyles } from './FiltersScreen.styles';
...
export const FiltersScreen: React.FC = () => {
  ...
  const handleCityZonesChange = (entries: CityZoneEntry[]) => {
    setCityZones(entries);
    setDraft((prev) => ({
      ...prev,
      city: entries.map((e) => e.city_id),
      zones: entries.flatMap((e) => e.zone_ids),
    }));
  };
      ...
      >
        {profileHousing === 'offering' ? (
          ...
          <>
            ...
            <FormSection title="Ciudad y zona" iconName="location-outline">
              <CityZoneSelector
                mode="multi"
                value={cityZones}
                onChange={handleCityZonesChange}
              />
            </FormSection>
...
const BudgetRange: React.FC<{
  ...
  styles: Styles;
}> = ({ minValue, maxValue, onDragStateChange, onChangeMin, onChangeMax, styles }) => {
