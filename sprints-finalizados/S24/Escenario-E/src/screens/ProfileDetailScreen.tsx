...
export const ProfileDetailScreen: React.FC<ProfileDetailScreenProps> = ({
        ...
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: () => {
            Alert.alert(
              ...
              'Se borraran todas tus fotos, matches, conversaciones y datos. ¿Seguro?',
              [
                { text: 'Cancelar', style: 'cancel' },
                {
  ...
  const preferredZoneLabels: string[] = (() => {
    if (preferredZones.length === 0) return [];
    const first = preferredZones[0];
    if (typeof first === 'object' && first !== null && 'city_id' in first) {
      return (preferredZones as { city_id: string; city_name: string; zone_names: string[] }[])
        .flatMap((entry) => entry.zone_names?.length ? entry.zone_names : [entry.city_name]);
    }
    return preferredZones as string[];
  })();
  ...
  const housingBadge =
    profile.housing_situation === 'seeking'
      ...
      ? `Tengo piso en ${
          (() => {
            if (preferredZones.length === 0) return 'zona preferida';
            const first = preferredZones[0];
            if (typeof first === 'object' && first !== null && 'city_name' in first) {
              return (first as { city_name: string }).city_name;
            }
            return preferredZoneLabels[0] ?? 'zona preferida';
          })()
        }`
      : null;
        ...
        <View style={styles.sectionsWrapper}>
          ...
          {activeTab === 'perfil' && (
            <>
              ...
              {/* Info rápida: ocupación, universidad, presupuesto, smoker, pets */}
              {(profile.occupation || profile.university || formatBudget() !== '-' || profile.smoker != null || profile.has_pets != null) && (
                <View style={styles.section}>
                  ...
                  <View style={styles.compactChips}>
                    ...
                    {profile.smoker != null ? (
                      <View style={[styles.compactChip, profile.smoker ? styles.warningChip : undefined]}>
                        <Text style={styles.compactChipEmoji}>
                          {profile.smoker ? '🚬' : '🚭'}
                        </Text>
                        <Text style={styles.compactChipText}>
                          {profile.smoker ? 'Fumador' : 'No fumador'}
                        </Text>
                      </View>
                    ) : null}
                    {profile.has_pets != null ? (
                      <View style={styles.compactChip}>
                        ...
                        <Text style={styles.compactChipText}>
                          {profile.has_pets ? 'Tiene mascotas' : 'Sin mascotas'}
                        </Text>
                      </View>
                    ) : null}
                  </View>
