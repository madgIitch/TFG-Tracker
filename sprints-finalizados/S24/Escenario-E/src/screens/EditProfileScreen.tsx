...
import { CityZoneSelector, type CityZoneEntry } from '../components/CityZoneSelector';
...
import { LIFESTYLE_TAGS, MAX_LIFESTYLE_TAGS } from '../constants/lifestyleTags';
import type {
...
export const EditProfileScreen: React.FC = () => {
  ...
  const [cityZones, setCityZones] = useState<CityZoneEntry[]>([]);
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      setLifestyleTags(data.lifestyle_tags || []);
      const rawZones = data.preferred_zones;
      if (Array.isArray(rawZones) && rawZones.length > 0 && typeof rawZones[0] === 'object' && rawZones[0] !== null && 'city_id' in rawZones[0]) {
        setCityZones(rawZones as CityZoneEntry[]);
      } else {
        setCityZones([]);
      }
      setIsActive(data.swipe_visibility_active !== false);
      const nextMin =
  ...
  const handleAddPhoto = async () => {
    ...
    try {
      ...
      await loadPhotos();
      Alert.alert('Foto añadida', 'Tu foto se ha subido correctamente.');
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const profileData: Partial<ProfileCreateRequest> = {
        ...
        preferred_zones: cityZones as unknown as string[],
        budget_min: presupuestoMinValue,
        budget_max: presupuestoMaxValue,
        swipe_visibility_active: isActive,
      };
      ...
      await setFilters({
        ...
        zones: cityZones.flatMap((e) => e.zone_ids),
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sectionBlock}>
          ...
          <Text style={styles.inlineLabel}>
            Estilo de vida ({lifestyleTags.length}/{MAX_LIFESTYLE_TAGS})
          </Text>
          <Text style={styles.inlineMeta}>
            Elige hasta {MAX_LIFESTYLE_TAGS} etiquetas que te describan
          </Text>
          <View style={styles.checkGrid}>
            {LIFESTYLE_TAGS.map((tag) => {
              const isActive = lifestyleTags.includes(tag.id);
              const isDisabled = !isActive && lifestyleTags.length >= MAX_LIFESTYLE_TAGS;
              return (
                <TouchableOpacity
                  key={tag.id}
                  style={[
                    ...
                    isDisabled && styles.checkItemDisabled,
                  ]}
                  onPress={() => {
                  ...
                  disabled={isDisabled}
                >
                  <Text style={styles.checkEmoji}>{tag.emoji}</Text>
                  <Text
                    style={[
                      styles.checkLabel,
                      isActive && styles.checkLabelActive,
                      isDisabled && styles.checkLabelDisabled,
                    ]}
                  >
                    {tag.label}
                  </Text>
        ...
        {/* Visibilidad */}
        <View style={styles.sectionBlock}>
          <Text style={styles.sectionTitleMuted}>Privacidad</Text>
          <View style={styles.sectionCard}>
            <View style={styles.themeRow}>
              <View style={{ flex: 1 }}>
                <Text style={styles.themeRowLabel}>Visible en el swipe</Text>
                <Text style={styles.themeRowSub}>
                  {isActive
                    ? 'Apareces en las recomendaciones de otros usuarios'
                    : 'Tu perfil está pausado y no apareces en ninguna búsqueda'}
                </Text>
              </View>
              <Switch
                value={isActive}
...
const BudgetRange: React.FC<{
  ...
  const theme = useTheme();
  const styles = useMemo(() => makeSliderStyles(theme), [theme]);
