...
export const ChatScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadAssignments = async () => {
      try {
        ...
        if (isMounted) setAssignmentsError(true);
      } finally {
        if (isMounted) {
  ...
  const assignRoom = async () => {
    if (!selectedRoomId || isAssigning) return;
    ...
    try {
      ...
      Alert.alert('Error', 'No se pudo asignar la habitación. Inténtalo de nuevo.');
    } finally {
          ...
          <View style={styles.bubbleMeta}>
            ...
            {isMine && item.status && (
              <Ionicons
                ...
                size={12}
                color={isMine ? 'rgba(255,255,255,0.7)' : theme.colors.primary}
              />
            )}
          </View>
        </View>
    ...
    <View style={styles.container}>
      ...
      {!isChatWithSeeker && (
        ...
        {loadingAssignments || loadingRooms ? (
          ...
          <Text style={styles.roommatesEmpty}>No se pudieron cargar las asignaciones.</Text>
        ) : canSeeAssignees ? (
          <>
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity
                style={[
                  styles.modalButton,
                  styles.modalConfirm,
                  (!selectedRoomId || isAssigning) && styles.modalButtonDisabled,
                ]}
                onPress={assignRoom}
                disabled={!selectedRoomId || isAssigning}
              >
                <Text style={styles.modalConfirmText}>
                  {isAssigning ? 'Asignando...' : assignTarget === 'owner' ? 'Asignarme' : 'Asignar'}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
