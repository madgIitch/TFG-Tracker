...
import { makeStyles } from './FlatExpensesScreen.styles';
...
export const FlatExpensesScreen: React.FC = () => {
  ...
  // Supabase Realtime: actualización automática cuando alguien añade un gasto
  useEffect(() => {
  ...
  const handleCreateExpense = async () => {
    ...
    const perPerson = parsedAmount / splitBetween.size;
    const splitNames = Array.from(splitBetween)
    ...
    await new Promise<void>((resolve) => {
      Alert.alert(
        'Confirmar gasto',
        `"${description.trim()}" · ${formatAmount(parsedAmount)} €\n\nPagado por: ${payerName}\nDividido entre: ${splitNames}\nA pagar cada uno: ${formatAmount(perPerson)} €`,
        [
          { text: 'Cancelar', style: 'cancel', onPress: () => resolve() },
          { text: 'Confirmar', style: 'default', onPress: () => { confirmed = true; resolve(); } },
        ]
      );
    });
    if (!confirmed) return;
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <KeyboardWrapper style={styles.modalOverlay} scrollable={false}>
