...
export const SwipeScreen: React.FC = () => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      profileService.getProfile().then((profile) => {
        if (profile) {
          setIsProfileActive(profile.swipe_visibility_active !== false);
        }
      }).catch(() => undefined);
    }, [])
  );
  ...
  const handleSwipe = (direction: 'left' | 'right') => {
    if (!currentProfile || !canSwipe || !isProfileActive) return;
    isSwiping = true;
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onMoveShouldSetPanResponder: (_, gesture) =>
        canSwipe && isProfileActive && Math.abs(gesture.dx) > 8 && Math.abs(gesture.dx) > Math.abs(gesture.dy) * 1.5,
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.content, { paddingTop: insets.top + 10 }]}>
        {!isProfileActive && (
          <TouchableOpacity
            style={[styles.inactiveBanner, { backgroundColor: theme.colors.warning }]}
            onPress={() => navigation.navigate('EditProfile')}
            activeOpacity={0.85}
          >
            <Ionicons name="pause-circle" size={18} color="#FFFFFF" />
            <Text style={styles.inactiveBannerText}>
              Tu perfil está pausado · Toca para activarlo
            </Text>
          </TouchableOpacity>
        )}
            ...
            {!canSwipe && currentProfile && (
              <View style={styles.limitOverlay}>
                ...
                <View style={styles.limitContent}>
                  <Ionicons name="flash-off" size={32} color={theme.colors.primary} />
      ...
      <GlassCard style={[styles.actionDock, { bottom: insets.bottom + 24 }]}>
        <View style={styles.actionRow}>
          <ActionButton
            ...
            disabled={!currentProfile || !canSwipe || !isProfileActive}
          />
          <ActionButton
            icon="heart"
