...
async function handler(req: Request): Promise<Response> {  
  ...
  try {  
    ...
    // Verificar que el dominio del email existe (DNS check)
    ...
    try {
      ...
      if (dnsData.Status === 3) {
        ...
        return new Response(
          JSON.stringify({ error: 'El dominio del email no existe' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }
    } catch {
      // Si el check DNS falla por error de red, dejamos pasar para no bloquear usuarios válidos
    }
