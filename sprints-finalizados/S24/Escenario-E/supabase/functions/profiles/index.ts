...
function validateProfileData(data: ProfileValidationData): {
  ...
  if (data.preferred_zone_ids !== undefined && !Array.isArray(data.preferred_zone_ids)) {
    errors.push('Preferred zone ids must be an array');
  } else if (data.preferred_zone_ids && !data.preferred_zone_ids.every((z) => typeof z === 'string')) {
    errors.push('All preferred zone ids must be strings');
  }

  if (data.preferred_city_id !== undefined && data.preferred_city_id !== null && typeof data.preferred_city_id !== 'string') {
  ...
  async (req: Request, payload: JWTPayload): Promise<Response> => {
    ...
    try {
      ...
      if (method === 'PATCH') {
        ...
        // Si se envía preferred_zones en formato nuevo [{city_id, zone_ids, ...}],
        // derivar preferred_city_id y preferred_zone_ids automáticamente
        if (Array.isArray(updates.preferred_zones) && updates.preferred_zones.length > 0 && typeof updates.preferred_zones[0] === 'object') {
          const cityZones = updates.preferred_zones as PreferredCityZone[];
          updates.preferred_city_id = cityZones[0]?.city_id ?? null;
          updates.preferred_zone_ids = cityZones.flatMap((c) => c.zone_ids ?? []);
        }
