...
import { createFiltersStyles } from '../styles/screens/FiltersScreen.styles';
...
export const FiltersScreen: React.FC = () => {
  ...
  const styles = useMemo(() => createFiltersStyles(theme), [theme]);
  ...
  const [lockedFeature, setLockedFeature] = useState<PremiumFeature>('advanced_filter_gender');
  const [upgradeModalVisible, setUpgradeModalVisible] = useState(false);
  const [cityOptions, setCityOptions] = useState<CityOption[]>([]);
  const [zoneOptionsByCity, setZoneOptionsByCity] = useState<Record<string, ZoneOption[]>>({});
  const [cityLabelById, setCityLabelById] = useState<Record<string, string>>({});
  const [zoneLabelById, setZoneLabelById] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    const loadInitialLocations = async () => {
      try {
        ...
        setCityLabelById(
          cities.reduce<Record<string, string>>((acc, city) => {
  ...
  const searchCities = async (query: string) => {
    try {
      ...
      console.error('[FiltersScreen] Error buscando ciudades:', error);
      setCityOptions([]);
    }
  };

  const searchZones = async (cityId: string, query: string) => {
    try {
      const zones = await locationService.searchZones(cityId, query);
      setZoneOptionsByCity((prev) => ({ ...prev, [cityId]: zones }));
      setZoneLabelById((prev) => ({
        ...prev,
      ...
      console.error('[FiltersScreen] Error buscando zonas:', error);
      setZoneOptionsByCity((prev) => ({ ...prev, [cityId]: [] }));
    }
  };
  ...
  const handleApply = async () => {
    ...
    const preferredCityId =
      draft.city.length > 0 ? draft.city[0] : null;

    try {
      await profileService.updateProfile({
        preferred_city_id: preferredCityId,
        preferred_zone_ids: draft.zones,
      });
    } catch (error) {
      console.warn('[FiltersScreen] No se pudo sincronizar ubicacion en perfil:', error);
    }
      ...
      >
        {profileHousing === 'offering' ? (
          ...
          <>
            ...
            <PremiumLockWrapper
              feature="advanced_filter_gender"
              featureName="Filtro de genero"
              onLockedPress={() => {
                setLockedFeature('advanced_filter_gender');
            ...
            <PremiumLockWrapper
              ...
              onLockedPress={() => {
                setLockedFeature('advanced_filter_age');
                setUpgradeModalVisible(true);
              }}
            >
            ...
            <FormSection title="Ciudad" iconName="business-outline">
              <SearchableSelect
                ...
                onSelect={(option) => {
                  setDraft((prev) => ({
                    ...prev,
                    city: prev.city.includes(option.id)
                      ? prev.city
                      : [...prev.city, option.id],
                  }));
                }}
              />
              <ChipGroup
                label="Ciudades seleccionadas"
                options={draft.city.map((cityId) => ({
                  id: cityId,
                  label: cityLabelById[cityId] ?? cityId,
                }))}
                selectedIds={draft.city}
                onSelect={(cityId) => {
                  setDraft((prev) => ({
                    ...prev,
                    city: prev.city.filter((item) => item !== cityId),
                    zones: prev.zones.filter((zoneId) =>
                      !(
                        zoneOptionsByCity[cityId] ?? []
                      ).some((zone) => zone.id === zoneId)
                    ),
                  }));
                }}
                multiline
            ...
            <FormSection title="Zonas" iconName="map-outline">
              {draft.city.map((cityId) => (
                <SearchableSelect
                  key={cityId}
                  label={`Agregar zona en ${cityLabelById[cityId] ?? cityId}`}
                  options={(zoneOptionsByCity[cityId] ?? []).map((zone) => ({
                    id: zone.id,
                    label: zone.name,
                  }))}
                  onSearch={(query) => {
                    void searchZones(cityId, query);
                  }}
                  onSelect={(option) => {
                    setDraft((prev) => ({
                      ...prev,
              ...
              <ChipGroup
                ...
                options={draft.zones.map((zoneId) => ({
                  id: zoneId,
                  label: zoneLabelById[zoneId] ?? zoneId,
                }))}
...
const RangeSlider: React.FC<{
  styles: ReturnType<typeof createFiltersStyles>;
  minBound: number;
  maxBound: number;
  step: number;
  minValue: number;
