...
import { createSwipeScreenStyles } from '../styles/screens/SwipeScreen.styles';

type GlassProps = {
  style?: object;
  children: React.ReactNode;
  styles: ReturnType<typeof createSwipeScreenStyles>;
};
...
const GlassCard: React.FC<GlassProps> = ({ style, children, styles }) => (
  <View style={[styles.glassCard, style]}>
    {children}
  </View>
);

const GlassChip: React.FC<GlassProps> = ({ style, children, styles }) => (
  <View style={[styles.glassChip, style]}>
    {children}
  </View>
);

const GlassButton: React.FC<GlassProps> = ({ style, children, styles }) => (
  ...
  return (
    ...
    >
      <Animated.View style={{ transform: [{ scale }] }}>
        <GlassButton style={styles.actionButton} styles={styles}>
...
export const SwipeScreen: React.FC = () => {
  ...
  const styles = useMemo(() => createSwipeScreenStyles(theme), [theme]);
  ...
  const [swipeVisibilityActive, setSwipeVisibilityActive] = useState(true);
  const [upgradeModalVisible, setUpgradeModalVisible] = useState(false);
  const [upgradeFeature, setUpgradeFeature] =
    useState<PremiumFeature>('daily_swipes_extended');
  ...
  const hasExtendedSwipes = canUse('daily_swipes_extended');
  const canSwipe = hasExtendedSwipes || swipesUsed < SWIPE_LIMIT;
  ...
  const mapProfileToSwipe = (
    ...
    return {
      ...
      preferredCityId: profile.preferred_city_id ?? null,
  ...
  useEffect(() => {
    const applyProfileFilters = async () => {
      ...
      try {
        ...
        setSwipeVisibilityActive(profile.swipe_visibility_active !== false);
        ...
        await setFilters({
          ...
          zones: profile.preferred_zone_ids ?? [],
          city: profile.preferred_city_id ? [profile.preferred_city_id] : [],
          roomCount: [],
          userType: [],
          ageRange: [18, 60],
  ...
  const loadProfiles = useCallback(async () => {
    ...
    try {
      const [recommendationsResult, matchesResult, rejectionsResult] =
      ...
      const rejections =
        rejectionsResult.status === 'fulfilled' ? rejectionsResult.value : [];

      if (matchesResult.status === 'rejected') {
        console.warn(
          '[SwipeScreen] No se pudieron cargar matches para exclusion:',
          matchesResult.reason
        );
      }
      if (rejectionsResult.status === 'rejected') {
        console.warn(
          '[SwipeScreen] No se pudieron cargar rechazos para exclusion:',
          rejectionsResult.reason
        );
      }
      ...
      console.error('[SwipeScreen] Error cargando recomendaciones:', error);
      setAllProfiles([]);
      setProfileError('No se pudieron cargar perfiles reales.');
    } finally {
      setLoadingProfiles(false);
    }
  }, [activeFilterCount, filters, lifestyleIdByLabel, profileFiltersApplied]);
  ...
  useEffect(() => {
    ...
    return unsubscribe;
  }, [navigation, profileFiltersApplied, loadProfiles]);
  ...
  const renderCard = (profile: SwipeProfile, index: number) => {
    ...
    return (
      ...
      >
        <GlassCard style={styles.profileCard} styles={styles}>
          <ImageBackground
            ...
            <View style={styles.profileOverlay}>
              <View style={styles.overlayContent}>
                ...
                <View style={styles.chipRow}>
                  {chips.map((chip) => (
                    <GlassChip key={chip} style={styles.profileChip} styles={styles}>
                ...
                >
                  <GlassButton style={styles.profileButton} styles={styles}>
      ...
      >
        <View style={styles.topBar}>
          ...
          <View style={styles.topActions}>
            <GlassChip style={styles.counterChip} styles={styles}>
              <Ionicons name="flash" size={14} color={theme.colors.text} />
              <Text style={styles.counterText}>
                {hasExtendedSwipes ? 'Ilimitados' : `${SWIPE_LIMIT - swipesUsed} libres`}
              </Text>
            </GlassChip>
            {!hasExtendedSwipes ? <PremiumBadge compact label="Free" /> : null}
            {!isOwnerOffering ? (
              <Pressable onPress={() => navigation.navigate('Filters')}>
                <View>
                  <GlassButton style={styles.filterButton} styles={styles}>
                    <Ionicons
        ...
        {!swipeVisibilityActive ? (
          <View style={styles.visibilityBanner}>
            ...
            <Text style={styles.visibilityBannerText}>
              Tu visibilidad en swipe esta pausada. Reactivala desde Perfil.
            </Text>
          </View>
        ) : null}
          ...
          <View style={styles.stack}>
            {loadingProfiles ? (
              ...
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptySubtitle}>
                  {!hasExtendedSwipes && !canSwipe
                    ? 'Has alcanzado tu limite diario. Mejora a Premium para seguir.'
                    : hasActiveFilters
                ...
                {!hasExtendedSwipes && !canSwipe ? (
                  <Pressable
                    onPress={() => {
                      setUpgradeFeature('daily_swipes_extended');
                      setUpgradeModalVisible(true);
                    }}
                  >
                    <GlassButton style={styles.clearFiltersButton} styles={styles}>
                      <Text style={styles.clearFiltersText}>Obtener Premium</Text>
                    </GlassButton>
                  </Pressable>
                ) : null}
                {hasActiveFilters && (
                ...
                {profileError && (
                  <>
                    ...
                    <Pressable onPress={() => loadProfiles().catch(() => undefined)}>
                      <GlassButton style={styles.clearFiltersButton} styles={styles}>
                        <Ionicons
                          ...
                          color={theme.colors.text}
                        />
                        <Text style={styles.clearFiltersText}>Reintentar</Text>
                      </GlassButton>
                    </Pressable>
                  </>
                )}
            ...
            {!canSwipe && currentProfile && (
              <View style={styles.limitOverlay}>
                ...
                >
                  <Text style={[styles.limitText, { marginTop: 6, textDecorationLine: 'underline' }]}>
                    Obtener Premium
                  </Text>
                </Pressable>
              </View>
...
const applyFilters = (
  ...
  return items.filter((profile) => {
    ...
    if (filters.zones.length > 0) {
      const matchesZone =
        profile.preferredZoneIds.some((zoneId) => filters.zones.includes(zoneId)) ||
        profile.preferredZones.some((zone) => filters.zones.includes(zone));
