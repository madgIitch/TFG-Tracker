...
import { createEditProfileStyles } from '../styles/screens/EditProfileScreen.styles';
import { GlassBackground } from '../components/GlassBackground';

const MAX_LIFESTYLE_TAGS = 5;
const lifestyleIdByLabel = new Map(
  ESTILO_VIDA_OPTIONS.map((option) => [option.label.toLowerCase(), option.id])
);
...
export const EditProfileScreen: React.FC = () => {
  ...
  const { scrollRef, handleInputFocus } = useKeyboardAutoScroll({ extraOffset: 92 });
  ...
  const [estiloVida, setEstiloVida] = useState<LifestyleTag[]>([]);
  ...
  const [preferredCityLabel, setPreferredCityLabel] = useState('');
  const [preferredZoneIds, setPreferredZoneIds] = useState<string[]>([]);
  const [preferredZoneLabelsById, setPreferredZoneLabelsById] = useState<
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      if (data.lifestyle_tags && data.lifestyle_tags.length > 0) {
        setEstiloVida(data.lifestyle_tags.slice(0, MAX_LIFESTYLE_TAGS));
      } else {
        setEstiloVida(
          data.lifestyle_preferences
            ? Object.values(data.lifestyle_preferences)
                .filter((value): value is string => Boolean(value))
                .map((value) => lifestyleIdByLabel.get(value.toLowerCase()))
                .filter((value): value is LifestyleTag => Boolean(value))
      ...
      setPreferredCityId(data.preferred_city_id ?? null);
      setPreferredZoneIds(data.preferred_zone_ids ?? []);
      const nextMin =
  ...
  useEffect(() => {
    const initCityAndZones = async () => {
      try {
        const cities = await locationService.searchCities('');
        setCityOptions(cities);
        if (preferredCityId) {
          const city = cities.find((item) => item.id === preferredCityId);
          if (city) {
            setPreferredCityLabel(city.name);
          }
          const zones = await locationService.searchZones(preferredCityId, '');
          setZoneOptions(zones);
          const labels = zones.reduce<Record<string, string>>((acc, zone) => {
            acc[zone.id] = zone.name;
            return acc;
          }, {});
          setPreferredZoneLabelsById((prev) => ({ ...prev, ...labels }));
        }
      } catch (error) {
        console.error('Error cargando ubicaciones:', error);
      }
    };
    void initCityAndZones();
  }, [preferredCityId]);

  const searchCities = async (query: string) => {
    try {
      const data = await locationService.searchCities(query);
      setCityOptions(data);
    } catch (error) {
  ...
  const searchZones = async (query: string) => {
    ...
    try {
      const data = await locationService.searchZones(preferredCityId, query);
      setZoneOptions(data);
      const labels = data.reduce<Record<string, string>>((acc, zone) => {
        acc[zone.id] = zone.name;
        return acc;
      }, {});
      setPreferredZoneLabelsById((prev) => ({ ...prev, ...labels }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!preferredCityId) {
        setFormWarning('Selecciona una ciudad preferida para mejorar tus recomendaciones.');
      }
      ...
      const profileData: Partial<ProfileCreateRequest> = {
        ...
        lifestyle_tags: estiloVida.slice(0, MAX_LIFESTYLE_TAGS),
      ...
      await setFilters({
        ...
        zones: preferredZoneIds,
        city: preferredCityId ? [preferredCityId] : [],
    ...
    >
      ...
      >
        <View style={styles.profileHeader}>
          ...
          {formWarning ? (
            <Text style={styles.validationText}>{formWarning}</Text>
          ) : null}
        </View>
        ...
        <View style={styles.sectionBlock}>
          ...
          />
          <Text style={styles.fieldCounter}>{biografia.length}/500</Text>
        ...
        <View style={styles.sectionBlock}>
          ...
          <Text style={styles.photoHint}>{estiloVida.length}/{MAX_LIFESTYLE_TAGS} seleccionados</Text>
          <View style={styles.checkGrid}>
            {ESTILO_VIDA_OPTIONS.map((option) => {
              const isActive = estiloVida.includes(option.id as LifestyleTag);
              return (
                <TouchableOpacity
                  key={option.id}
                  style={[styles.checkItem, isActive && styles.checkItemActive]}
                  onPress={() => {
                    setEstiloVida((prev) => {
                      if (prev.includes(option.id as LifestyleTag)) {
                        return prev.filter((interes) => interes !== (option.id as LifestyleTag));
                      }
                      if (prev.length >= MAX_LIFESTYLE_TAGS) {
                        Alert.alert(
                          'Limite alcanzado',
                          `Puedes seleccionar hasta ${MAX_LIFESTYLE_TAGS} estilos de vida.`
                        );
                        return prev;
                      }
                      return [...prev, option.id as LifestyleTag];
                    });
                  }}
                >
                  <View
        ...
        <View style={styles.sectionBlock}>
          ...
          <SearchableSelect
            ...
            valueLabel={preferredCityLabel || undefined}
            options={cityOptions.map((city) => ({ id: city.id, label: city.name }))}
            onSearch={searchCities}
            onSelect={(option) => {
              if (preferredZoneIds.length > 0 && option.id !== preferredCityId) {
                Alert.alert(
                  'Zonas reiniciadas',
                  'Al cambiar de ciudad se eliminan las zonas seleccionadas.'
                );
              }
              setPreferredCityId(option.id);
              setPreferredCityLabel(option.label);
              setPreferredZoneIds([]);
              setPreferredZoneLabelsById({});
              setZoneOptions([]);
              setZonas([]);
            }}
          />
          <SearchableSelect
            label="Agregar zona"
            placeholder={preferredCityId ? 'Buscar zona...' : 'Selecciona ciudad primero'}
            ...
            onSelect={(option) => {
              ...
              setPreferredZoneLabelsById((prev) => ({
                ...prev,
                [option.id]: option.label,
              }));
              setZonas((prev) =>
                prev.includes(option.label) ? prev : [...prev, option.label]
              );
            }}
          ...
          <ChipGroup
            ...
            options={preferredZoneIds.map((zoneId) => ({
              id: zoneId,
              label: preferredZoneLabelsById[zoneId] ?? zoneId,
            }))}
            selectedIds={preferredZoneIds}
            onSelect={(zoneId) => {
              setPreferredZoneIds((prev) => prev.filter((id) => id !== zoneId));
              const legacyLabel = preferredZoneLabelsById[zoneId];
              if (legacyLabel) {
                setZonas((prev) => prev.filter((item) => item !== legacyLabel));
              }
            }}
            multiline
          />
...
const BudgetRange: React.FC<{
  styles: ReturnType<typeof createEditProfileStyles>;
  minValue: number;
  maxValue: number;
  onChangeMin: (value: number) => void;
  onChangeMax: (value: number) => void;
}> = ({ styles, minValue, maxValue, onChangeMin, onChangeMax }) => {
