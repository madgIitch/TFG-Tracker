...
import type { Flat, Room, RoomExtras, FlatInvitationCode } from '../types/room';
import type { RoomAssignment } from '../types/roomAssignment';
import type { Gender } from '../types/gender';
import { createRoomManagementStyles } from '../styles/screens/RoomManagementScreen.styles';
...
export const RoomManagementScreen: React.FC = () => {
  ...
  const styles = useMemo(() => createRoomManagementStyles(theme), [theme]);
  ...
  const [invitationCode, setInvitationCode] = useState<FlatInvitationCode | null>(null);
  ...
  const loadRooms = useCallback(async () => {
    try {
      ...
      setLoadError('No se pudieron cargar las habitaciones.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const loadFlats = useCallback(async () => {
    try {
      ...
      setLoadError('No se pudieron cargar los pisos.');
    }
  }, []);
  ...
  useEffect(() => {
    const ownerId = authContext?.user?.id ?? null;
    if (!ownerId) return;
    ...
    const subscribe = async () => {
      channel = await createRealtimeSubscription(
        buildRealtimeChannelName(
          'room-management',
          selectedFlatId ? `flat-${selectedFlatId}` : 'owner',
          ownerId,
          ownerId
        ),
        [
    ...
    subscribe().catch((error) => {
      console.error('Error suscribiendo realtime en RoomManagement:', error);
    });
  ...
  }, [authContext?.user?.id, loadFlats, loadRooms, selectedFlatId]);
  ...
  const loadInvitationCode = useCallback(async () => {
    if (!selectedInviteRoomId) {
      setInvitationCode(null);
      return;
    }
    try {
      setLoadingInvitationCode(true);
      const code = await roomService.getRoomInvitationCode(selectedInviteRoomId);
      setInvitationCode(code);
    } catch (error) {
      console.error('Error cargando codigo de invitacion:', error);
      setInvitationCode(null);
    } finally {
      setLoadingInvitationCode(false);
    }
  ...
  const handleOpenFlatSettlements = () => {
    if (!selectedFlatId) {
      Alert.alert('Aviso', 'Selecciona un piso primero.');
      return;
    }
  ...
  const handleGenerateInvitationCode = async () => {
    if (!selectedInviteRoomId) {
      Alert.alert('Aviso', 'Selecciona una habitacion primero.');
      return;
    }
    try {
      setLoadingInvitationCode(true);
      const code = await roomService.createRoomInvitationCode(selectedInviteRoomId);
      setInvitationCode(code);
      Alert.alert('Listo', 'Codigo de invitacion generado');
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (Clipboard?.setString) {
          Clipboard.setString(invitationCode.code);
          copied = true;
        }
      } catch {
        copied = false;
      ...
      await Share.share({
        message: `Codigo de invitacion HomiMatch: ${invitationCode.code}`,
      });
    } catch (error) {
      console.error('Error copiando codigo:', error);
      Alert.alert('Error', 'No se pudo copiar el codigo');
    }
  };
  ...
  const handleShareInvitationCode = async () => {
    ...
    try {
      await Share.share({
        message: `Te invito a una habitacion en HomiMatch. Usa este codigo: ${invitationCode.code}`,
      });
    } catch (error) {
  ...
  const invitationCodeExpiresAt = invitationCode?.expires_at
    ? new Date(invitationCode.expires_at).toLocaleString('es-ES')
    : null;
  ...
  const filteredRooms = selectedFlatId
    ...
    : rooms;
  const inviteCandidateRooms = filteredRooms.filter((room) => {
    ...
    const hasAcceptedAssignment = assignmentsForRoom.some(
      (assignment) => assignment.status === 'accepted'
    );
    return !isCommonArea && Boolean(room.is_available) && !hasAcceptedAssignment;
  });

  useEffect(() => {
    if (inviteCandidateRooms.length === 0) {
      setSelectedInviteRoomId(null);
      return;
    }
    if (!selectedInviteRoomId || !inviteCandidateRooms.some((room) => room.id === selectedInviteRoomId)) {
      setSelectedInviteRoomId(inviteCandidateRooms[0].id);
    }
  }, [inviteCandidateRooms, selectedInviteRoomId]);
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      {loading ? (
        ...
        <View style={styles.loadingContainer}>
          ...
          >
            <Text style={styles.createFlatButtonText}>Reintentar</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <ScrollView
            ...
            <>
              ...
              <View style={styles.summaryRail}>
                ...
                <View style={styles.summaryItem}>
                  <Text style={styles.summaryValue}>
                    {filteredRooms.filter((room) => room.is_available).length}
                  </Text>
                ...
                <View style={styles.summaryItem}>
                  <Text style={styles.summaryValue}>
                    {Object.values(roomAssignments)
                      .flat()
                      .filter((assignment) => assignment.status === 'accepted').length}
                  </Text>
              ...
              <FormSection title="Ubicacion del piso" iconName="location-outline">
                <Text style={styles.detailText}>
                  {selectedFlat?.address ?? 'Sin direccion'}
                </Text>
                <Text style={styles.detailMeta}>
                  {selectedFlat?.city ?? 'Sin ciudad'}
                  {selectedFlat?.district ? ` - ${selectedFlat.district}` : ''}
                </Text>
                <TouchableOpacity
                  style={styles.inlineAction}
                  onPress={() =>
                    selectedFlatId
                      ? navigation.navigate('CreateFlat', { flatId: selectedFlatId })
                      : Alert.alert('Aviso', 'Selecciona un piso primero.')
                  }
                >
                  <Text style={styles.inlineActionText}>Editar ubicacion</Text>
                </TouchableOpacity>
              </FormSection>
              ...
              <FormSection title="Codigo de invitacion" iconName="key-outline">
                {inviteCandidateRooms.length > 0 ? (
                  <>
                    <Text style={styles.flatSelectorLabel}>Habitacion</Text>
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                      <View style={styles.flatChips}>
                        {inviteCandidateRooms.map((room) => {
                          const isActive = room.id === selectedInviteRoomId;
                          return (
                            <TouchableOpacity
                              key={room.id}
                              style={[styles.flatChip, isActive && styles.flatChipActive]}
                              onPress={() => setSelectedInviteRoomId(room.id)}
                            >
                              <Text
                                style={[styles.flatChipText, isActive && styles.flatChipTextActive]}
                              >
                                {room.title}
                              </Text>
                            </TouchableOpacity>
                  ...
                  <Text style={styles.detailEmpty}>
                    Crea una habitacion privada para generar codigo de invitacion.
                  </Text>
                )}

                {loadingInvitationCode ? (
                  <Text style={styles.detailEmpty}>Cargando codigo...</Text>
                ) : invitationCode && selectedInviteRoomId ? (
                  <>
                    <Text style={styles.invitationCodeValue} selectable>
                      {invitationCode.code}
                    </Text>
                    <Text style={styles.detailMeta}>
                      Caduca: {invitationCodeExpiresAt ?? 'Sin fecha'}
                    </Text>
                    <Text style={styles.detailMeta}>
                      Plazas disponibles: {invitationCode.available_slots}
                    </Text>
                    <View style={styles.flatToolsRow}>
                      <TouchableOpacity
                        style={styles.flatToolButton}
                        onPress={handleCopyInvitationCode}
                      >
                        <Ionicons name="copy-outline" size={16} color="#7C3AED" />
                        <Text style={styles.flatToolText}>Copiar</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                        style={styles.flatToolButton}
                      ...
                      >
                        ...
                        <Text style={styles.flatToolText}>Compartir</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                        style={styles.flatToolButton}
                      ...
                      >
                        ...
                        <Text style={styles.flatToolText}>Regenerar</Text>
                      </TouchableOpacity>
                    </View>
                  </>
                ) : (
                  <>
                    <Text style={styles.detailEmpty}>
                      Genera un codigo para invitar a esta habitacion sin pasar por swipes.
                    </Text>
                    <TouchableOpacity
                      style={styles.inlineAction}
                      onPress={handleGenerateInvitationCode}
                      disabled={!selectedInviteRoomId}
                    >
                      <Text style={styles.inlineActionText}>Generar codigo</Text>
                    </TouchableOpacity>
                  </>
                )}
              </FormSection>
              ...
              <FormSection title="Gastos compartidos" iconName="wallet-outline">
                <Text style={styles.detailEmpty}>
                  Registra gastos comunes y calcula deudas pendientes entre companeros.
                </Text>
                <View style={styles.flatToolsRow}>
                  <TouchableOpacity
                    style={styles.flatToolButton}
                    onPress={handleOpenFlatExpenses}
                  >
                    <Ionicons name="receipt-outline" size={16} color="#7C3AED" />
                    <Text style={styles.flatToolText}>Ver gastos</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={styles.flatToolButton}
                    onPress={handleOpenFlatSettlements}
                  >
                    <Ionicons name="swap-horizontal-outline" size={16} color="#7C3AED" />
                    <Text style={styles.flatToolText}>Liquidaciones</Text>
                  </TouchableOpacity>
                </View>
              </FormSection>
