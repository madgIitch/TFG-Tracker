...
type RoomInsertPayload = RoomCreateRequest & {
  owner_id: string;
};
...
async function canAccessOwnerInventory(requesterId: string, ownerId: string): Promise<boolean> {
  ...
  const { data: matchRows } = await supabaseClient
    ...
    .select('id')
    .or(
      `(user_a_id.eq.${requesterId},user_b_id.eq.${ownerId}),(user_a_id.eq.${ownerId},user_b_id.eq.${requesterId})`
    )
    .limit(1)
  ...
  // Public read access for published inventory:
  // if the owner has at least one available room, their inventory can be viewed.
  const { data: publishedRooms } = await supabaseClient
    .from('rooms')
    .select('id')
    .eq('owner_id', ownerId)
  ...
  if (publishedRooms && publishedRooms.length > 0) {
    return true
  }
  ...
  return Boolean(assignmentRows && assignmentRows.length > 0)
}
...
async function countAvailableSlotsForRoom(roomId: string): Promise<number> {
  const { data: room, error: roomError } = await supabaseClient
    .from('rooms')
    .select('id, is_available, extras:room_extras(capacity)')
    .eq('id', roomId)
    .maybeSingle();

  if (roomError || !room) return 0;
  const roomRow = room as RoomAvailabilityRow;
  if (roomRow.is_available !== true) return 0;
  ...
  const capacityValue = roomRow.extras?.[0]?.capacity;
  const capacity =
    typeof capacityValue === 'number' && Number.isFinite(capacityValue)
      ? Math.max(1, Math.floor(capacityValue))
      : 1;
  const acceptedCount = assignments?.length ?? 0;
  return Math.max(0, capacity - acceptedCount);
}
...
async function enrichFlatLegacyFields(
  payload: Partial<FlatCreateRequest> & { city?: string; district?: string }
): Promise<Partial<FlatCreateRequest> & { city?: string; district?: string; province_code?: string | null }> {
  const result: Partial<FlatCreateRequest> & {
    city?: string;
    district?: string;
  ...
  if (payload.city_id) {
    const { data: cityData } = await supabaseClient
      .from('cities')
      .select('name, ine_municipio, ref_ine')
      .eq('id', payload.city_id)
      .maybeSingle();
    if (cityData?.name) {
      result.city = cityData.name as string;
    }
    const ineMunicipio = (cityData?.ine_municipio as string | null | undefined) ?? null;
    const refIne = (cityData?.ref_ine as string | null | undefined) ?? null;
    if (ineMunicipio && ineMunicipio.length >= 2) {
      result.province_code = ineMunicipio.slice(0, 2);
    } else if (refIne) {
      const digits = refIne.replace(/\D/g, '');
      result.province_code = digits.length >= 2 ? digits.slice(0, 2) : null;
    }
  }
  ...
  if (payload.district_id) {
    const { data: districtData } = await supabaseClient
      .from('city_places')
      .select('name')
      .eq('id', payload.district_id)
      .maybeSingle();
    if (districtData?.name) {
      result.district = districtData.name as string;
    }
  }
...
async function validateLocationConsistency(
  data: Pick<FlatValidationData, 'city_id' | 'district_id'>
): Promise<string[]> {
  const errors: string[] = [];
  if (!data.city_id) return errors;
  ...
  const { data: cityData } = await supabaseClient
    ...
    .maybeSingle();
  if (!cityData?.id) {
    errors.push('city_id must reference an existing city');
    return errors;
  }
  ...
  if (data.district_id) {
    const { data: districtData } = await supabaseClient
      ...
      .maybeSingle();
    if (!districtData?.id) {
      errors.push('district_id must reference an existing city_place');
    } else if (districtData.city_id !== data.city_id) {
      errors.push('district_id must belong to city_id');
    }
  }
 ...
 * Crear nuevo room
 */
async function createRoom(roomData: RoomInsertPayload): Promise<Room> {
  const { data: targetFlat, error: flatError } = await supabaseClient
    .from('flats')
    .select('id, owner_id')
    .eq('id', roomData.flat_id)
    .single();

  if (flatError || !targetFlat) {
    throw new Error('Flat not found');
  }

  if (targetFlat.owner_id !== roomData.owner_id) {
    throw new Error('Unauthorized: You can only create rooms in your own flats');
  }
...
async function updateRoom(roomId: string, userId: string, updates: Partial<Room>): Promise<Room> {
  ...
  if (updates.flat_id && updates.flat_id !== existingRoom.flat_id) {
    throw new Error(
      'Cannot move room to another flat. Create a new room in the target flat to keep location consistency.'
    );
  }
...
async function deleteRoom(roomId: string, userId: string): Promise<void> {
  ...
  await supabaseClient.from('flat_invitation_codes').delete().eq('room_id', roomId)
  await supabaseClient.from('room_assignments').delete().eq('room_id', roomId)
  await supabaseClient.from('room_interests').delete().eq('room_id', roomId)
...
function validateFlatData(data: FlatValidationData): { isValid: boolean; errors: string[] } {
  ...
  if (data.city_id !== undefined) {
    if (typeof data.city_id !== 'string' || data.city_id.trim().length < 2) {
...
function validateRoomData(
  ...
  if (!options.partial && (!data.flat_id || typeof data.flat_id !== 'string')) {
    errors.push('Flat ID is required')
  } else if (data.flat_id !== undefined && typeof data.flat_id !== 'string') {
    errors.push('Flat ID must be a string')
  }

  if (!options.partial && data.price_per_month == null) {
    errors.push('Price per month is required')
  } else if (
    data.price_per_month !== undefined &&
    (typeof data.price_per_month !== 'number' || data.price_per_month <= 0)
  ) {
    errors.push('Price per month must be greater than 0')
  }

  if (!options.partial && !data.available_from) {
    errors.push('Available from date is required')
  } else if (data.available_from !== undefined && typeof data.available_from !== 'string') {
    errors.push('Available from date must be a string')
  }
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      if (ownerIdParam && ownerIdParam !== userId) {
        const canAccess = await canAccessOwnerInventory(userId, ownerIdParam)
        if (!canAccess) {
          return new Response(
            JSON.stringify({ error: 'Forbidden owner_id scope' }),
            {
              status: 403,
    ...
    if (method === 'POST') {
      ...
      if (type === 'flat') {
        let flatData: FlatCreateRequest = {
          ...body,
          owner_id: userId // Forzar el owner_id del token
        }

        flatData = await enrichFlatLegacyFields(flatData) as FlatCreateRequest;

        const validation = validateFlatData(flatData)
        const locationErrors = await validateLocationConsistency(flatData)
        validation.errors.push(...locationErrors)
      ...
      if (type === 'room') {
        const roomData: RoomInsertPayload = {
          ...body,
          owner_id: userId // Forzar el owner_id del token
        }
    ...
    if (method === 'PATCH') {
      ...
      if (type === 'flat') {
        const enrichedUpdates = await enrichFlatLegacyFields(updates);
        const validation = validateFlatData(enrichedUpdates)
        const locationErrors = await validateLocationConsistency(enrichedUpdates)
        validation.errors.push(...locationErrors)
        validation.isValid = validation.errors.length === 0
        if (!validation.isValid) {
          return new Response(
        ...
        const updatedFlat = await updateFlat(resourceId, userId, enrichedUpdates)
      ...
      if (type === 'room') {
        const validation = validateRoomData(updates, { partial: true })
        if (!validation.isValid) {
          return new Response(
            JSON.stringify({
              error: 'Validation failed',
