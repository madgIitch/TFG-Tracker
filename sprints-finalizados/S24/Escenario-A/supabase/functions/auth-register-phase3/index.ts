...
async function resolveInvitationJoinPlan(
  ...
  const { data: roomData, error: roomError } = await supabaseClient
    .from('rooms')
    .select('id, flat_id, owner_id, is_available, extras:room_extras(capacity)')
  ...
  const room = roomData as RoomAvailabilityRow
...
async function completeInvitationJoin(
  ...
  if (roomError || !roomData?.id || !roomData.is_available) {
    throw new Error('La habitacion ya no esta disponible')
  }

  const capacityValue = (roomData as { extras?: Array<{ capacity?: number | null }> }).extras?.[0]?.capacity
  const capacity =
    typeof capacityValue === 'number' && Number.isFinite(capacityValue)
      ? Math.max(1, Math.floor(capacityValue))
      : 1
  ...
  if ((existingAccepted?.length ?? 0) >= capacity) {
    throw new Error('La habitacion ya no esta disponible')
  }
  ...
  if (((existingAccepted?.length ?? 0) + 1) >= capacity) {
...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (tempData.is_google_user) {
      const googleUserId =
        typeof (body as { google_user_id?: string }).google_user_id === 'string'
          ? (body as { google_user_id?: string }).google_user_id!.trim()
          : ''

      if (!googleUserId) {
        return new Response(
          JSON.stringify({ error: 'google_user_id is required for Google registrations' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }

      const { data: googleAuthUser, error: googleAuthUserError } =
        await supabaseClient.auth.admin.getUserById(googleUserId)
      if (googleAuthUserError || !googleAuthUser?.user) {
        return new Response(
          JSON.stringify({ error: 'Google user not found' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }

      if (googleAuthUser.user.email !== tempData.email) {
        return new Response(
          JSON.stringify({ error: 'Google user email mismatch' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }

      const userRecord = {
        id: googleAuthUser.user.id,
      ...
      if (userError) {
        return new Response(
          JSON.stringify({ error: 'Failed to upsert Google user record' }),
      ...
      if (profileError) {
        return new Response(
          JSON.stringify({ error: 'Failed to upsert Google profile' }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }
      ...
      if (invitationPlan) {
        try {
          await completeInvitationJoin(supabaseClient, googleAuthUser.user.id, invitationPlan)
        } catch (joinError) {
          return new Response(
            JSON.stringify({
      ...
      const response: AuthResponse = {
        ...
        user: {
          ...
          birth_date: body.birth_date,
          gender: tempData.gender,
          created_at: googleAuthUser.user.created_at,
        },
      }
