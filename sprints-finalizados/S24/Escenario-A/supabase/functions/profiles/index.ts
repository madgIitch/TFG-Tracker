...
async function mapPreferredZonesFromIds(zoneIds: string[]): Promise<string[]> {
  ...
  const byId = new Map((data as { id: string; name: string }[]).map((item) => [item.id, item.name]));
  return zoneIds.map((id) => byId.get(id)).filter((name): name is string => Boolean(name));
}

async function validatePreferredLocationConsistency(data: ProfileValidationData): Promise<string[]> {
  const errors: string[] = [];

  if (data.preferred_city_id) {
    const { data: cityData } = await supabaseClient
      .from('cities')
      .select('id')
      .eq('id', data.preferred_city_id)
      .maybeSingle();
    if (!cityData?.id) {
      errors.push('Preferred city id must reference an existing city');
      return errors;
    }
  }

  if (data.preferred_zone_ids && data.preferred_zone_ids.length > 0) {
    const { data: zonesData, error } = await supabaseClient
      .from('city_places')
      .select('id, city_id')
      .in('id', data.preferred_zone_ids);

    if (error || !zonesData || zonesData.length !== data.preferred_zone_ids.length) {
      errors.push('Preferred zone ids must reference existing city places');
      return errors;
    }

    if (
      data.preferred_city_id &&
      zonesData.some((zone) => zone.city_id !== data.preferred_city_id)
    ) {
      errors.push('Preferred zone ids must belong to preferred city id');
    }
  }
...
function validateProfileData(data: ProfileValidationData): {
  ...
  if (
    data.lifestyle_tags &&
    (!Array.isArray(data.lifestyle_tags) ||
      !data.lifestyle_tags.every((item) => typeof item === 'string'))
  ) {
    errors.push('Lifestyle tags must be an array of strings');
  }
  if (data.lifestyle_tags && data.lifestyle_tags.length > 5) {
    errors.push('Lifestyle tags cannot exceed 5 entries');
  }
  ...
  if (
    data.preferred_city_id !== undefined &&
    data.preferred_city_id !== null &&
    typeof data.preferred_city_id !== 'string'
  ) {
    errors.push('Preferred city id must be a string or null');
  }
  if (
    data.preferred_zone_ids &&
    (!Array.isArray(data.preferred_zone_ids) ||
      !data.preferred_zone_ids.every((item) => typeof item === 'string'))
  ) {
    errors.push('Preferred zone ids must be an array of strings');
  }
  ...
  if (
    data.swipe_visibility_active !== undefined &&
    typeof data.swipe_visibility_active !== 'boolean'
  ) {
    errors.push('Swipe visibility active must be a boolean');
  }
  ...
  async (req: Request, payload: JWTPayload): Promise<Response> => {
    ...
    try {
      ...
      if (method === 'POST') {
        ...
        body.id = userId;
        if (Array.isArray(body.preferred_zone_ids) && body.preferred_zone_ids.length > 0) {
          body.preferred_zones = await mapPreferredZonesFromIds(body.preferred_zone_ids);
        }

        const validation = validateProfileData(body);
        const locationErrors = await validatePreferredLocationConsistency(body);
      ...
      if (method === 'PATCH') {
        ...
        if (Array.isArray(updates.preferred_zone_ids)) {
          updates.preferred_zones = await mapPreferredZonesFromIds(updates.preferred_zone_ids);
        }
        ...
        const locationErrors = await validatePreferredLocationConsistency({
          ...existingProfile,
          ...updates,
        });
        validation.errors.push(...locationErrors);
