...
interface ExistingAssignmentRow extends AssignmentRow {
  room?: { owner_id?: string };
}

type RoomCapacityRow = {
  id: string;
  is_available?: boolean | null;
  extras?: Array<{ capacity?: number | null }> | null;
};
...
async function countAvailableSlotsForRoom(roomId: string): Promise<number> {
  ...
  const typedRoom = room as RoomCapacityRow;
  if (typedRoom.is_available !== true) return 0;
  ...
  const capacityValue = typedRoom.extras?.[0]?.capacity;
  const capacity =
    typeof capacityValue === 'number' && Number.isFinite(capacityValue)
      ? Math.max(1, Math.floor(capacityValue))
      : 1;
  return Math.max(0, capacity - data.length);
}

async function hasAcceptedAssignment(roomId: string): Promise<boolean> {
  return (await countAvailableSlotsForRoom(roomId)) <= 0;
}

async function rejectOtherOffers(roomId: string, keepAssignmentId: string): Promise<void> {
  await supabaseAdmin
    .from('room_assignments')
    .update({ status: 'rejected' })
    .eq('room_id', roomId)
    .eq('status', 'offered')
    .neq('id', keepAssignmentId);
}

async function getChatIdByMatchId(matchId: string): Promise<string | null> {
  const { data, error } = await supabaseAdmin
    .from('chats')
    .select('id')
    .eq('match_id', matchId)
  ...
  withAuth(async (req: Request, payload: JWTPayload): Promise<Response> => {
    ...
    if (req.method === 'POST') {
      ...
      if (matchId) {
        ...
        void (async () => {
          const tokens = await getTokensForUsers([assigneeId]);
          if (tokens.length === 0) return;

          const chatId = await getChatIdByMatchId(matchId);
          const assignee = (data.assignee ?? {}) as { display_name?: string; avatar_url?: string };
          if (chatId) {
            fireAndForgetPush({
              tokens,
              title: 'Nueva oferta de habitacion',
              body: 'Tienes una nueva oferta en un chat activo.',
              data: {
                screen: 'Chat',
                chatId,
                name: assignee.display_name?.trim() || 'Chat',
                avatarUrl: assignee.avatar_url?.trim() || '',
              },
            });
            return;
          }
          ...
          fireAndForgetPush({
            tokens,
            title: 'Nueva oferta de habitacion',
            body: 'Tienes una nueva oferta. Revisa tus matches.',
            data: {
              screen: 'Matches',
            },
          });
        })().catch((pushError) => {
          console.error('[room-assignments] push trigger error:', pushError);
        });
      ...
      if ((await countAvailableSlotsForRoom(roomId)) <= 0) {
        await supabaseAdmin
          .from('rooms')
    ...
    if (req.method === 'PATCH') {
      ...
      const typedExisting = existing as ExistingAssignmentRow;
      const isAssignee = typedExisting.assignee_id === userId;
      const isOwner = typedExisting.room?.owner_id === userId;
      ...
      if (status === 'accepted' && typedExisting.status !== 'accepted') {
        const alreadyAssigned = await hasAcceptedAssignment(typedExisting.room_id);
        if (alreadyAssigned) {
          return new Response(JSON.stringify({ error: 'Room already assigned' }), {
            status: 409,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      ...
      if (status === 'accepted') {
        const remainingSlots = await countAvailableSlotsForRoom(typedExisting.room_id);
        if (remainingSlots <= 0) {
          await supabaseAdmin
            .from('rooms')
            .update({ is_available: false })
            .eq('id', typedExisting.room_id);
          await rejectOtherOffers(typedExisting.room_id, assignmentId);
        }
      }
      if (status === 'rejected' && typedExisting.status === 'accepted') {
        await supabaseAdmin
          .from('rooms')
