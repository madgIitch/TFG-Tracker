...
const mapCityFromPlaceRow = (row: GenericRow): LocationCity | null => {
  const id = getCityId(row);
  const name = getCityName(row);
  if (!id || !name) return null;

  return {
    id,
    name,
    ...(asNumber(row.latitude ?? row.lat) != null
      ? { latitude: asNumber(row.latitude ?? row.lat) }
      : {}),
    ...(asNumber(row.longitude ?? row.lng ?? row.lon) != null
      ? { longitude: asNumber(row.longitude ?? row.lng ?? row.lon) }
      : {}),
  };
};
...
async function searchCities(q: string, limit: number): Promise<LocationCity[]> {
  ...
  if (!placesError && placesData) {
    const byId = new Map<string, LocationCity>();
    (placesData as GenericRow[])
      .map(mapCityFromPlaceRow)
    ...
    const fromPlaces = Array.from(byId.values())
  ...
  return (citiesData as GenericRow[])
