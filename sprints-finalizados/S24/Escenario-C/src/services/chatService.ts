...
class ChatService {
  ...
  async getMatches(): Promise<Match[]> {
    ...
    if (!response.ok) {
      throw new Error(
        await this.extractErrorMessage(response, 'Error al obtener matches')
      );
    }
  ...
  async getChats(): Promise<Chat[]> {
    ...
    if (!response.ok) {
      throw new Error(
        await this.extractErrorMessage(response, 'Error al obtener chats')
      );
    }
    ...
    return chatsWithMessages.sort(
      (a, b) => toMillis(b.lastMessageAtISO) - toMillis(a.lastMessageAtISO)
    );
  }
  ...
  async getChatByMatchId(matchId: string): Promise<Chat | null> {
    ...
    if (!response.ok) {
      throw new Error(
        await this.extractErrorMessage(response, 'Error al obtener chat')
      );
    }
    ...
    const currentUserId = await this.getCurrentUserId();
    const isUserA = currentUserId && chat.match?.user_a_id === currentUserId;
  ...
  async createChat(matchId: string): Promise<Chat> {
    ...
    if (!response.ok) {
      throw new Error(
        await this.extractErrorMessage(response, 'Error al crear chat')
      );
    }
  ...
  async getMessages(chatId: string): Promise<Message[]> {
    ...
    if (!response.ok) {
      throw new Error(
        await this.extractErrorMessage(response, 'Error al obtener mensajes')
      );
    }
  ...
  async sendMessage(chatId: string, text: string): Promise<Message> {
    ...
    if (!response.ok) {
      throw new Error(
        await this.extractErrorMessage(response, 'Error al enviar mensaje')
      );
    }
    ...
    if (!response.ok) {
      throw new Error(
        await this.extractErrorMessage(
          response,
          'Error al marcar mensajes como leidos'
        )
      );
    }
  }
