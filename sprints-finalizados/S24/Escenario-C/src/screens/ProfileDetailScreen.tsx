...
import { createStyles } from '../styles/screens/ProfileDetailScreen.styles';
...
const lifestyleTagLabelById = new Map([
...
const isRenderableImageUrl = (value: unknown): value is string =>
...
export const ProfileDetailScreen: React.FC<ProfileDetailScreenProps> = ({
  ...
  const [updatingSwipeVisibility, setUpdatingSwipeVisibility] = useState(false);
  ...
  const handleDeleteAccount = () => {
    ...
    Alert.alert(
      'Eliminar cuenta',
      'Esta accion borra permanentemente tu cuenta y todos tus datos. No se puede deshacer.',
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          text: 'Continuar',
  ...
  useEffect(() => {
    if (routeProfile) {
      ...
      setSwipeVisible(routeProfile.social_links?.swipe_visible !== false);
      setLoading(false);
  ...
  const loadProfile = async () => {
    try {
      ...
      setProfile(data);
      setSwipeVisible(data?.social_links?.swipe_visible !== false);
    } catch (error) {
  ...
  const handleToggleSwipeVisibility = async (value: boolean) => {
    if (!isOwnProfile || !profile || updatingSwipeVisibility) {
      return;
    }

    const previous = swipeVisible;
    setSwipeVisible(value);
    setUpdatingSwipeVisibility(true);
    try {
      ...
      setProfile(updated);
      setSwipeVisible(updated.social_links?.swipe_visible !== false);
    } catch (error) {
      setSwipeVisible(previous);
      ...
      setUpdatingSwipeVisibility(false);
    }
  };
  ...
  const lifestyleItems =
    profile.lifestyle_tags && profile.lifestyle_tags.length > 0
      ? profile.lifestyle_tags.map(
          (tag) => lifestyleTagLabelById.get(tag) ?? tag
        )
      : (
  ...
  const flatZoneLabels = Array.from(
    new Set(
      flats
        .map((flat) =>
          typeof flat.district === 'string' && flat.district.trim().length > 0
            ? flat.district.trim()
            : null
        )
  ...
  const primaryFlatCity =
    typeof flats[0]?.city === 'string' && flats[0].city.trim().length > 0
      ? flats[0].city.trim()
      : null;
  const displayZoneLabels =
    profile.housing_situation === 'offering' && flatZoneLabels.length > 0
      ? flatZoneLabels
      : preferredZoneLabels;
  ...
  const housingBadge =
    profile.housing_situation === 'seeking'
      ...
      ? `Tengo piso en ${displayZoneLabels[0] ?? primaryFlatCity ?? 'zona preferida'}`
      : null;
  ...
  const normalizedProfilePhotos = profilePhotos
    .map((photo) => {
      const altSignedUrl = (photo as ProfilePhoto & { signed_url?: string }).signed_url;
      const normalizedUrl =
        (typeof photo.signedUrl === 'string' && photo.signedUrl) ||
        (typeof altSignedUrl === 'string' && altSignedUrl) ||
        (typeof photo.path === 'string' && photo.path.startsWith('http') ? photo.path : '');

      return {
        ...photo,
        signedUrl: normalizedUrl,
      };
    })
    .filter((photo) => isRenderableImageUrl(photo.signedUrl));

  const carouselPhotos =
    normalizedProfilePhotos.length > 0
      ? normalizedProfilePhotos
      : resolvedAvatarUrl
      ? [
          {
            id: 'avatar',
  ...
  const cityLabel =
    profile.housing_situation === 'offering'
      ? displayZoneLabels[0] ?? primaryFlatCity ?? profile.university ?? 'Sin ciudad'
      : preferredZoneLabels[0] ?? profile.university ?? 'Sin ciudad';
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView
        ...
        contentContainerStyle={[
          styles.contentInner,
          !isOwnProfile && !isFromMatch && styles.contentInnerWithActions,
        ]}
      ...
      >
          ...
          <View style={styles.visibilityCard}>
            <View style={styles.visibilityRow}>
              <View style={styles.visibilityInfo}>
                <Text style={styles.visibilityTitle}>Visible en swipe</Text>
                <Text style={styles.visibilityText}>
                  Si lo desactivas, no apareceras en recomendaciones de otras personas.
                </Text>
              </View>
              <Switch
                value={swipeVisible}
                onValueChange={handleToggleSwipeVisibility}
                disabled={updatingSwipeVisibility}
                trackColor={{ false: theme.colors.border, true: theme.colors.primaryLight }}
                thumbColor={swipeVisible ? theme.colors.primary : theme.colors.surfaceLight}
              />
            </View>
          </View>
        )}
        {isOwnProfile && !swipeVisible && (
          <View style={styles.visibilityBanner}>
            <Ionicons name="pause-circle-outline" size={18} color={theme.colors.warning} />
            <Text style={styles.visibilityBannerText}>
              Tu perfil esta pausado en swipe. No apareces en recomendaciones.
            </Text>
          </View>
        )}
        {shouldShowFlatTab && (
          ...
          <View style={styles.heroCard}>
            <FlatList
              ...
              onMomentumScrollEnd={(event) => {
                ...
                setActiveProfilePhotoIndex(index);
              }}
              renderItem={({ item }) => (
                <TouchableOpacity
                  activeOpacity={0.95}
                ...
                >
                  <Image
                    source={{ uri: item.signedUrl, cache: 'reload' }}
                    style={styles.heroImage}
                    resizeMode="cover"
                    onError={(event) => {
                      console.warn('[ProfileDetail] Hero image failed to load', {
                        photoId: item.id,
                        uri: item.signedUrl,
            ...
            <View style={styles.heroOverlay}>
              ...
              <View style={styles.heroIdentityRow}>
                ...
                <View style={styles.heroCityChip}>
              ...
              <View style={styles.heroChipsRow}>
                {memberSinceYear ? (
                  <View style={styles.heroChip}>
                    <Text style={styles.heroChipText}>Desde {memberSinceYear}</Text>
                  </View>
                ) : null}
                {housingBadge ? (
                  <View style={styles.heroChip}>
                ...
                <View style={styles.heroDots}>
                  {carouselPhotos.map((photo, index) => (
                    <View
                      ...
                      style={[
                        styles.heroDot,
                        index === activeProfilePhotoIndex && styles.heroDotActive,
                      ]}
                    />
                  ))}
                </View>
        ...
        {lifestyleItems.length > 0 && (
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              ...
              <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Estilos de vida</Text>
            </View>
            <View style={styles.detailCard}>
              <View style={styles.compactChips}>
                {lifestyleItems.map((item, index) => (
                  <View key={`${item}-${index}`} style={styles.compactChip}>
                    <Text style={styles.compactChipText}>{item}</Text>
                  </View>
                ))}
              </View>
            </View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Lo que busca</Text>
          </View>
          <View style={styles.detailCard}>
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>Situación</Text>
              <Text style={styles.detailValue}>{housingBadge ?? 'No especificado'}</Text>
            </View>
            ...
            {displayZoneLabels.length > 0 && (
              <View style={[styles.detailRow, styles.detailRowSpacing]}>
                <Text style={styles.detailLabel}>Zonas</Text>
                <Text style={styles.detailValue}>{displayZoneLabels.join(', ')}</Text>
              </View>
            )}
          </View>
        </View>
        ...
        {summaryChips.length > 0 && (
          <View style={styles.section}>
            <Text style={styles.sectionMutedTitle}>Estilo y contexto</Text>
            <View style={styles.compactChips}>
