...
import { createStyles } from '../styles/screens/ChatScreen.styles';
...
export const ChatScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (!ownerId) return;
    let isMounted = true;
    const userIsOwner = Boolean(currentUserId && ownerId === currentUserId);

    const loadOwnerRooms = async () => {
      try {
        setLoadingRooms(true);
        const rooms = userIsOwner
  ...
  const upsertMessage = (next: Message) => {
    setMessages((prev) => {
      const existingIndex = prev.findIndex((message) => message.id === next.id);
      if (existingIndex !== -1) {
        const updated = [...prev];
        updated[existingIndex] = { ...updated[existingIndex], ...next };
        return updated.sort(byMessageDateAsc);
      }
      return [...prev, next].sort(byMessageDateAsc);
    });
  };

  const handleIncomingMessageUI = (next: Message) => {
  ...
  useEffect(() => {
    const loadMessages = async () => {
      try {
        ...
        setMessages(data.sort(byMessageDateAsc));
  ...
  const groupedItems = useMemo<ChatListItem[]>(() => {
    const items: ChatListItem[] = [];
    let currentDayKey: string | null = null;

    messages.forEach((message) => {
      const dayKey = toDayKey(message.createdAtISO);
  ...
  const handleScroll = (event: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    const nextNearBottom = distanceToBottom <= 56;
    isNearBottomRef.current = nextNearBottom;
    if (nextNearBottom && pendingIncomingCount > 0) {
      setPendingIncomingCount(0);
    }
  };
        ...
        >
          ...
          <View style={styles.bubbleMeta}>
            <Text
              style={[
                styles.bubbleTime,
                isMine ? styles.bubbleStatus : styles.bubbleStatusOther,
              ]}
            >
              {item.createdAt}
            </Text>
  ...
  const canSendMessage = inputValue.trim().length > 0 && !sendingMessage;
  const showInitialMessageState = loadingMessages && messages.length === 0;
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      {showInitialMessageState ? (
        <View style={styles.centerState}>
          ...
          <Text style={styles.centerStateText}>Cargando conversación...</Text>
        </View>
      ) : (
        <FlatList
          ref={listRef}
          ...
          ListEmptyComponent={
            <View style={styles.centerState}>
              <Text style={styles.centerStateText}>Aún no hay mensajes</Text>
              <Text style={styles.centerStateSubtext}>
                {messagesError ?? 'Envía el primero para empezar la conversación.'}
              </Text>
            </View>
          }
        />
      ...
      <KeyboardAvoidingView
        ...
        keyboardVerticalOffset={Platform.OS === 'ios' ? Math.max(insets.top, 12) : 12}
      >
...
const byMessageDateAsc = (a: Message, b: Message) => {
  const left = toMillis(a.createdAtISO);
  const right = toMillis(b.createdAtISO);
  return left - right;
};
