...
import { createStyles } from '../styles/screens/SwipeScreen.styles';
...
const toCompatibilityPercentage = (rawScore: number) => {
  ...
  return (
    ...
    >
      <Animated.View style={{ transform: [{ scale }] }}>
        <GlassButton style={styles.actionButton} styles={styles} blurType={blurType}>
...
export const SwipeScreen: React.FC = () => {
  ...
  const canBypassDailyLimit = canUseFeature('daily_swipe_limit_bypass');
  const canSwipe = canBypassDailyLimit || swipesUsed < SWIPE_LIMIT;
  const remainingSwipeText = canBypassDailyLimit
    ? 'Ilimitados'
    : `${Math.max(SWIPE_LIMIT - swipesUsed, 0)} libres`;
  const activeFilterCount = countActiveFilters(filters);
  const hasActiveFilters = activeFilterCount > 0;
  const isOwnerOffering = profileHousing === 'offering';
  const getCompatibilityBadgeColor = React.useCallback(
    (score: number) => {
  ...
  const bottomDockOffset = Math.max(insets.bottom + 8, 16);
  const contentBottomPadding = bottomDockOffset + 84;
  ...
  const advanceCard = () => {
    const nextCount = canBypassDailyLimit
      ? swipesUsed + 1
      : Math.min(swipesUsed + 1, SWIPE_LIMIT);
  ...
  const mapProfileToSwipe = (
    ...
    const compatibilityPercentage =
      typeof compatibilityScore === 'number'
        ? toCompatibilityPercentage(compatibilityScore)
        : 0;
  ...
  useEffect(() => {
    const applyProfileFilters = async () => {
      ...
      try {
        ...
        setIsSwipeVisible(profile.social_links?.swipe_visible !== false);
        ...
        const mappedLifestyle =
          profile.lifestyle_tags && profile.lifestyle_tags.length > 0
            ...
            : (profile.lifestyle_preferences
                ...
                .map((label) => lifestyleIdByLabel.get(label) ?? label)
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      const refreshSwipeVisibility = async () => {
        try {
          const profile = await profileService.getProfile();
          if (!isMounted) return;
          setIsSwipeVisible(profile?.social_links?.swipe_visible !== false);
      ...
      refreshSwipeVisibility().catch(() => undefined);
    ...
    const loadProfiles = async () => {
      ...
      try {
        ...
        const mapped = recommendations
          .map((rec) => {
            const mappedProfile = mapProfileToSwipe(
              rec.profile,
      ...
      >
        <View style={styles.profileCard}>
          ...
          <View style={styles.profileImageOverlay}>
            ...
            {profile.compatibilityPercentage >= 0 && (
              <View
                style={[
                  styles.compatibilityBadge,
                  {
                    backgroundColor: getCompatibilityBadgeColor(
            ...
            <View style={styles.profileOverlay}>
              ...
              <View style={styles.overlayContent}>
                ...
                >
                  <GlassButton style={styles.profileButton} styles={styles} blurType={theme.isDark ? 'dark' : 'light'}>
  ...
  return (
    <View style={styles.container}>
      <View
        style={[
          styles.content,
          { paddingTop: topInsetPadding, paddingBottom: contentBottomPadding },
        ]}
      >
        <GlassBar style={styles.topBar} styles={styles} blurType={theme.isDark ? 'dark' : 'light'}>
        ...
        {!isSwipeVisible && (
          <View style={styles.visibilityBanner}>
            ...
            <Text style={styles.visibilityBannerText}>
              Tu perfil esta pausado: no apareces en recomendaciones.
            </Text>
          </View>
        )}
          ...
          <View style={styles.stack}>
            {loadingProfiles ? (
              <View style={styles.emptyState}>
                <Ionicons name="hourglass" size={36} color={theme.colors.textSecondary} />
              ...
              <View style={styles.emptyState}>
                <Ionicons name="heart-dislike" size={36} color={theme.colors.textSecondary} />
            ...
            {!canSwipe && currentProfile && (
              <View style={styles.limitOverlay}>
                <Text style={styles.limitText}>Limite diario free alcanzado</Text>
                <Pressable
                  style={styles.limitUpgradeButton}
                  onPress={() => setIsUpgradeModalVisible(true)}
                >
                  <Text style={styles.limitUpgradeButtonText}>Mejorar a Premium</Text>
                </Pressable>
              </View>
            )}
          </View>
      ...
      <GlassCard style={[styles.actionDock, { bottom: bottomDockOffset }]} styles={styles} blurType={theme.isDark ? 'dark' : 'light'}>
...
const applyFilters = (
  ...
  const lifestyleLabels = filters.lifestyle
    .map((id) => normalizeLifestyleToken(lifestyleLabelById.get(id) ?? id))
    .filter(Boolean);
  ...
  return items.filter((profile) => {
    ...
    if (lifestyleLabels.length > 0 && profile.housing !== 'offering') {
      if (profile.lifestyle.length > 0) {
        const profileLifestyleTokens = profile.lifestyle.map((chip) =>
          normalizeLifestyleToken(chip)
        );
        const matchesLifestyle = profile.lifestyle.some((chip) =>
          profileLifestyleTokens.includes(normalizeLifestyleToken(chip)) &&
          lifestyleLabels.includes(normalizeLifestyleToken(chip))
        );
