...
describe('authService', () => {
  ...
  it('registerPhase1 normalizes email to lowercase and trims whitespace', async () => {
    ...
    await authService.registerPhase1({
      email: '  USER@Test.com  ',
      password: '123456',
    } as Phase1Data);
  ...
  it('registerPhase1 surfaces DNS domain error from backend', async () => {
    (globalThis.fetch as jest.Mock).mockResolvedValue({
      ok: false,
      status: 400,
      json: async () => ({
    ...
    await expect(
      authService.registerPhase1({
        email: 'a@dominionoexiste123.com',
        password: '123456',
      } as Phase1Data)
    ).rejects.toThrow(
      'El dominio del email no existe. Revisa el correo e intentalo de nuevo.'
    );
  });

  it('registerPhase1 surfaces invalid format error from backend', async () => {
    (globalThis.fetch as jest.Mock).mockResolvedValue({
      ok: false,
      status: 400,
      json: async () => ({
    ...
    ).rejects.toThrow('El formato del email no es valido.');
  });
  ...
  it('deleteAccount calls account-delete endpoint and clears local auth storage', async () => {
    mockStorage.getItem.mockResolvedValue('token-delete');
    (globalThis.fetch as jest.Mock).mockResolvedValue({
      ok: true,
      status: 200,
    ...
    expect(String(url)).toContain('/account-delete');
    expect(init.method).toBe('DELETE');
    expect(init.headers.Authorization).toBe('Bearer token-delete');
    expect(mockStorage.removeItem).toHaveBeenCalledWith('authToken');
    expect(mockStorage.removeItem).toHaveBeenCalledWith('authUser');
  });

  it('deleteAccount surfaces backend error and keeps local session data', async () => {
    mockStorage.getItem.mockResolvedValue('token-delete');
    (globalThis.fetch as jest.Mock).mockResolvedValue({
      ok: false,
      status: 500,
    ...
    await expect(authService.deleteAccount()).rejects.toThrow('Deletion failed');
