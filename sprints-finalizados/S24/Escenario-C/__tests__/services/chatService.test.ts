...
describe('chatService', () => {
  ...
  it('getMatches returns only accepted matches', async () => {
    (globalThis.fetch as jest.Mock).mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => ({
        data: [
          ...
          },
          {
            ...
            user_b_id: 'u3',
            status: 'pending',
            user_b: { id: 'u3', display_name: 'Luz', avatar_url: null },
          },
        ],
      }),
    });
  ...
  it('getChatDetails maps lifestyle_tags in counterpart profile', async () => {
    (globalThis.fetch as jest.Mock).mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => ({
    ...
    const { chatService } = require('../../src/services/chatService');
    const chat = await chatService.getChatDetails('chat-lifestyle');

    expect(chat?.profile).toBeTruthy();
