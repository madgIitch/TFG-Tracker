...
export const ProfileDetailScreen: React.FC<ProfileDetailScreenProps> = ({
    ...
    if (routeProfile) {
      ...
      if (routeProfile.id && routeProfile.id !== currentUserId) {
        profilePhotoService
          ...
          .then((data) => {
            console.log('[ProfileDetail] External profile photos fetched:', {
  ...
  const loadPhotos = async () => {
    try {
      ...
      console.log('[ProfileDetail] Own profile photos fetched:', {
        count: data.length,
        first: data[0],
      });
      setProfilePhotos(data);
  ...
  useEffect(() => {
    if (profilePhotos.length === 0) {
      console.log('[ProfileDetail] No profile photos returned by API');
      return;
    }

    const invalid = profilePhotos.filter((photo) => {
      ...
      return !isRenderableImageUrl(candidate);
    });

    if (invalid.length > 0) {
      console.warn('[ProfileDetail] Photos with non-renderable URL detected:', invalid);
    }

    const firstValidUrl = profilePhotos
      .map((photo) => {
        const altSignedUrl = (photo as ProfilePhoto & { signed_url?: string }).signed_url;
        return (
          (typeof photo.signedUrl === 'string' && photo.signedUrl) ||
      ...
      .find((url) => isRenderableImageUrl(url));

    if (firstValidUrl) {
      console.log('[ProfileDetail] Probing first image URL with Image.getSize', {
        url: firstValidUrl,
      });
      Image.getSize(
        firstValidUrl,
        (width, height) => {
          console.log('[ProfileDetail] Image.getSize success', {
            url: firstValidUrl,
            width,
            height,
          });
        ...
        (error) => {
          console.warn('[ProfileDetail] Image.getSize failed', {
            url: firstValidUrl,
            error,
          });
        }
              ...
              renderItem={({ item }) => (
                ...
                >
                  <Image
                    ...
                    resizeMode="cover"
                    onLoadStart={() => {
                      console.log('[ProfileDetail] Hero image load start', {
                        photoId: item.id,
                        uri: item.signedUrl,
                      });
                    }}
                    onLoad={() => {
                      console.log('[ProfileDetail] Hero image loaded', {
                        photoId: item.id,
                        uri: item.signedUrl,
                      });
                    }}
                    onLoadEnd={() => {
                      console.log('[ProfileDetail] Hero image load end', {
                        photoId: item.id,
                        uri: item.signedUrl,
                      });
                    }}
