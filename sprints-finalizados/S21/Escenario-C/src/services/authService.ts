...
class AuthService {
    ...
    try {
      ...
      if (profileResponse.status === 404) {
        ...
        const profilePayload = (await profileResponse.json()) as {
          data?: { gender?: string | null; birth_date?: string | null } | null;
        };
        const profile = profilePayload?.data ?? null;
        const hasNames = Boolean(mappedUser.first_name && mappedUser.last_name);
        const hasGender = Boolean(profile?.gender);
        const hasBirthDate = Boolean(profile?.birth_date);
        isNewUser = !(hasNames && hasGender && hasBirthDate);
      } else {
        isNewUser = false;
      }
    } catch {
  ...
  async registerPhase1(data: Phase1Data): Promise<TempRegistration> {
    ...
    try {
      ...
      if (!response.ok) {
        let message = 'Error en fase 1 del registro';
        try {
          const payload = (await response.json()) as { error?: string; details?: string };
          if (payload?.error === 'Email domain does not exist') {
            message = 'El dominio del email no existe. Revisa el correo e intentalo de nuevo.';
          } else if (payload?.error === 'Invalid email format') {
  ...
  async registerPhase3(
    tempToken: string,
    data: Phase3Data,
    sessionTokens?: { accessToken?: string; refreshToken?: string | null }
