...
import { createFlatExpensesStyles } from '../styles/screens/FlatExpensesScreen.styles';
...
export const FlatExpensesScreen: React.FC = () => {
  ...
  const styles = useMemo(() => createFlatExpensesStyles(theme), [theme]);
  ...
  const scrollBottomPadding = insets.bottom + 92;
  ...
  useEffect(() => {
    ...
    const scheduleRefresh = () => {
      ...
      refreshTimer = setTimeout(() => {
        ...
        if (!mounted) return;
        loadExpenses().catch((error) => {
          console.error('Error recargando gastos por realtime:', error);
        });
      }, 250);
    };
  ...
  }, [currentUserId, loadExpenses, selectedFlatId]);
  ...
  const renderExpense = (expense: FlatExpense) => {
    ...
    return (
      <View key={expense.id} style={[styles.glassCard, styles.expenseCard]}>
      ...
      <Modal visible={showCreateModal} transparent animationType="slide" onRequestClose={closeCreateModal}>
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            >
              <View style={[styles.glassCard, styles.modalCard]}>
