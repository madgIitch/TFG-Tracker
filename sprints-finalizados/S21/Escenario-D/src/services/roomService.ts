...
class RoomService {
  ...
  async getTenantFlatAndRooms(userId: string): Promise<{ flats: Flat[], rooms: Room[] }> {
    const headers = await this.getAuthHeaders();
    const baseHeaders = {
      ...headers,
    ...
    try {
      const assignmentsRes = await fetch(`${API_CONFIG.SUPABASE_URL}/rest/v1/room_assignments?select=room_id&assignee_id=eq.${userId}&status=eq.accepted`, {
        headers: baseHeaders
      });
      const assignmentsData = await assignmentsRes.json();
      const roomIds = Array.isArray(assignmentsData) ? assignmentsData.map((a: any) => a.room_id) : [];

      if (roomIds.length === 0) return { flats: [], rooms: [] };

      const roomsRes = await fetch(`${API_CONFIG.SUPABASE_URL}/rest/v1/rooms?select=*,flat:flats(*)&id=in.(${roomIds.join(',')})`, {
        headers: baseHeaders
      });
      ...
      rooms.forEach((room: any) => {
        if (room.flat && !flatMap.has(room.flat.id)) {
          flatMap.set(room.flat.id, room.flat);
        }
      });
      const flats = Array.from(flatMap.values());

      return { flats, rooms };
    } catch (error) {
      console.error('Error fetching tenant flat and rooms:', error);
      return { flats: [], rooms: [] };
    }
  }

  async getInvitationCode(roomId: string): Promise<{ code: string } | null> {
    const headers = await this.getAuthHeaders();
    const response = await fetch(`${API_CONFIG.FUNCTIONS_URL}/room-invitation-codes?room_id=${roomId}`, {
      method: 'GET',
      headers,
    });
    if (!response.ok) return null;
    const data = await response.json();
    return data.data ? { code: data.data.code } : null;
  }

  async generateInvitationCode(roomId: string): Promise<{ code: string }> {
    const headers = await this.getAuthHeaders();
    const response = await fetch(`${API_CONFIG.FUNCTIONS_URL}/room-invitation-codes`, {
      method: 'POST',
      headers,
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'Error al generar el código');
    }
    const data = await response.json();
