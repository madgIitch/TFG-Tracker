...
import { View, Alert, Text, Image, KeyboardAvoidingView, Platform, ScrollView } from 'react-native';
...
import { getCommonStyles } from '../styles/common.styles';
import { getStyles } from '../styles/screens/RegisterScreen.styles';

type RootStackParamList = {
  Login: undefined;
  Register: { prefilledGoogleUser?: any } | undefined;
  Main: undefined;
};

type RegisterScreenNavigationProp = StackNavigationProp<RootStackParamList, 'Register'>;

export const RegisterScreen: React.FC = () => {  const navigation = useNavigation<RegisterScreenNavigationProp>();
  ...
  const [invitationData, setInvitationData] = useState<{ invitationCode?: string } | null>(null);
  const [loading, setLoading] = useState(false);
  const route = useRoute<RouteProp<RootStackParamList, 'Register'>>();

  useEffect(() => {
    const prefilledGoogleUser = route.params?.prefilledGoogleUser;
    if (prefilledGoogleUser && currentPhase === 1) {
      setTempRegistration({
        tempToken: 'google_' + prefilledGoogleUser.id,
        email: prefilledGoogleUser.email,
        isGoogleUser: true,
      });
      setCurrentPhase(2);
    }
  }, [route.params?.prefilledGoogleUser]);
  ...
  const handlePhase4 = (data: { invitationCode?: string }) => {
