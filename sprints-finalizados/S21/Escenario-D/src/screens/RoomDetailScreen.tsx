...
import { getStyles } from '../styles/screens/RoomDetailScreen.styles';
...
export const RoomDetailScreen: React.FC = () => {  const theme = useTheme();
  ...
  useEffect(() => {
    ...
    const channel = supabaseClient
      .channel(`room_detail:room:${room.id}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'room_assignments', filter: `room_id=eq.${room.id}` },
        () => { if (isMounted) refreshRoom(); }
      )
      .on(
        'postgres_changes',
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
        ...
        {photos.length > 0 && (
          <View style={styles.heroContainer}>
            <FlatList
              ...
              renderItem={({ item }) => (
                <View style={{ width: Dimensions.get('window').width, height: 460 }}>
                  <Image source={{ uri: item.signedUrl }} style={styles.heroImage} resizeMode="cover" />
                </View>
              )}
            />
            {photos.length > 1 && (
                ...
                <View style={styles.heroCapsuleContent}>
                  ...
                  {flat && (
                    <Text style={styles.heroLocation}>
                      <Ionicons name="location-sharp" size={14} color="#FFF" /> {flat.city}{flat.district ? `, ${flat.district}` : ''}
                    </Text>
                  )}
                  <View style={styles.heroBadgesRow}>
                    {!isCommonArea && roomState.price_per_month != null && (
                      <View style={[styles.heroBadgeClear, styles.heroBadgeAccent]}>
                        <Text style={styles.heroBadgeTextClear}>{roomState.price_per_month} EUR/mes</Text>
                      </View>
                    )}
                    {typeLabel && (
                      <View style={styles.heroBadgeClear}>
                        <Text style={styles.heroBadgeTextClear}>{typeLabel}</Text>
                      </View>
                    )}
                    {roomState.size_m2 != null && (
                      <View style={styles.heroBadgeClear}>
                        <Text style={styles.heroBadgeTextClear}>{roomState.size_m2} m²</Text>
                      </View>
                    )}
                  </View>
                </View>
      ...
      {!isCommonArea && !isAssigned && currentUserId !== room.owner_id && (
        <View style={[styles.bottomActions, { bottom: Math.max(insets.bottom, 24) }]}>
          <View style={styles.priceContainer}>
            <Text style={[styles.priceValue, { color: theme.colors.primary }]}>
              {roomState.price_per_month}<Text style={[styles.priceUnit, { color: theme.colors.textSecondary }]}> €/mes</Text>
            </Text>
            {statusLabel && (
              <Text style={[styles.statusText, { color: statusLabel === 'Disponible' ? theme.colors.primary : '#EF4444' }]}>
                {statusLabel}
              </Text>
            )}
          </View>
          <TouchableOpacity style={[styles.actionButton, { backgroundColor: theme.colors.primary }]}>
            <Text style={styles.actionButtonText}>
              {statusLabel === 'Disponible' ? 'Contactar' : 'Avisar'}
            </Text>
          </TouchableOpacity>
        </View>
      )}
