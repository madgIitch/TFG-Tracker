...
async function createMatch(matchData: {
  ...
  try {
    ...
    if (match.status === 'accepted') {
      const tokens = await getTokensForUsers([match.user_a_id, match.user_b_id]);
      if (tokens.length > 0) {
        await sendPush({
          tokens,
          title: 'Nuevo match',
    ...
    console.error('[matches] push error:', pushError);
  }

  return data as Match;
}

function isDuplicateMatchError(errorMessage: string): boolean {
  return (
...
async function ensureChatForMatch(matchId: string): Promise<void> {
  ...
  if (existingError) {
    throw new Error(`Failed to verify chat: ${existingError.message}`);
  }
...
async function findMatchesBetweenUsers(
  userAId: string,
  userBId: string
): Promise<Match[]> {
  const { data, error } = await supabaseClient
  ...
  if (error) {
    throw new Error(`Failed to fetch existing match pair: ${error.message}`);
  }
  ...
  async (req: Request, payload: JWTPayload): Promise<Response> => {
    ...
    try {
      ...
      if (method === 'POST') {
        ...
        const existingMatches = await findMatchesBetweenUsers(
          matchData.user_a_id,
          matchData.user_b_id
        );

        const acceptedMatch = existingMatches.find(
        ...
        const reversePending = existingMatches.find(
          (match) =>
            match.user_a_id === matchData.user_b_id &&
            match.user_b_id === matchData.user_a_id &&
            match.status === 'pending'
        );
        if (reversePending) {
          const updatedMatch = await updateMatch(reversePending.id, userId, {
            status: 'accepted',
          });
        ...
        const sameDirectionPending = existingMatches.find(
          (match) =>
            match.user_a_id === matchData.user_a_id &&
            match.user_b_id === matchData.user_b_id &&
            match.status === 'pending'
        );
        if (sameDirectionPending) {
          const response: ApiResponse<Match> = { data: sameDirectionPending };
          return new Response(JSON.stringify(response), {
            status: 200,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        ...
        try {
          ...
          if (!isDuplicateMatchError(createErrorMessage)) {
            throw createError;
          }

          const latestMatches = await findMatchesBetweenUsers(
            matchData.user_a_id,
            matchData.user_b_id
          );
          const resolved =
            latestMatches.find((existing) => existing.status === 'accepted') ??
            latestMatches.find(
              (existing) =>
                existing.user_a_id === matchData.user_a_id &&
                existing.user_b_id === matchData.user_b_id
            ) ??
            latestMatches[0];
          ...
          if (resolved.status === 'accepted') {
            await ensureChatForMatch(resolved.id);
          }
      ...
      if (isDuplicateMatchError(errorMessage)) {
