...
function buildRoomSearchQuery(filters: RoomFilters) {
 ...
 */  
function validateRoomCreateData(data: RoomValidationData): { isValid: boolean; errors: string[] } {  
  const errors: string[] = []  
...
function validateRoomUpdateData(data: RoomValidationData): { isValid: boolean; errors: string[] } {
  const errors: string[] = []
  ...
  if (data.price_per_month !== undefined && (typeof data.price_per_month !== 'number' || data.price_per_month < 0)) {
    errors.push('Price per month must be a non-negative number')
  }

  if (data.available_from !== undefined && (typeof data.available_from !== 'string' || data.available_from.trim().length === 0)) {
    errors.push('Available from must be a valid date')
  }

  if (data.size_m2 !== undefined && (typeof data.size_m2 !== 'number' || data.size_m2 < 5)) {
    errors.push('Size m2 must be at least 5')
  }

  if (data.is_available !== undefined && typeof data.is_available !== 'boolean') {
    errors.push('is_available must be boolean')
  }
  ...
  try {  
    ...
    if (method === 'POST') {  
      ...
      if (resourceId === 'search') {
        ...
        const per_page = Math.min(Number(body.per_page) > 0 ? Number(body.per_page) : 20, 50)
        ...
        const { data, count, error } = await buildRoomSearchQuery(filters)
          .order('created_at', { ascending: false })
      ...
      if (type === 'room') {  
        ...
        const validation = validateRoomCreateData(roomData)  
        if (!validation.isValid) {  
          return new Response(  
            JSON.stringify({       
              error: 'Validation failed',       
    ...
    if (method === 'PATCH') {  
      ...
      if (type === 'room') {  
        const validation = validateRoomUpdateData(updates)  
        if (!validation.isValid) {  
          return new Response(  
            JSON.stringify({       
              error: 'Validation failed',       
